...
public class Scenario4_LieuInexistant
{
    ...
    public void Should_refuser_la_pose_car_le_lieu_n_existe_pas_et_n_inscrire_aucun_slot_When_un_Parent_place_un_enfant_dans_un_lieu_absent_des_lieux_du_foyer()
    {
        ...
        var lieux = new FakeReferentielActivites();
    ...
    private static PoserSlotHandler Handler(FakeReferentielActivites lieux, out FakeSlotRepository slots)
    {
    ...
    {
        var handler = Handler(new FakeReferentielActivites(), out _);
    ...
    {
        var handler = Handler(new FakeReferentielActivites().AvecActivite("ecole"), out _);
    ...
    {
        var handler = Handler(new FakeReferentielActivites(), out var slots);
        var commande = new SlotBuilder().DansLieu("ancienne-creche").Build();
