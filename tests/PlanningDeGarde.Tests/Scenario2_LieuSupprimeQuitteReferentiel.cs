...
//   port d'écriture IEditeurLieux — il cesse d'être énuméré ET n'est plus acceptable à la pose. Borne
...
public class Scenario2_LieuSupprimeQuitteReferentiel
{
    ...
    public void Acceptation_Should_Retirer_nounou_du_referentiel_et_refuser_toute_nouvelle_pose_tout_en_preservant_le_slot_deja_pose_When_le_parent_supprime_nounou()
    {
        ...
        var suppression = supprimer.Handle(new SupprimerLieuCommand(Nounou));

        Assert.True(suppression.EstSucces);
        // « nounou » ne figure plus au référentiel des lieux du foyer
        Assert.DoesNotContain(referentiel.EnumererLieux(), lieu => lieu.Id == Nounou);
    ...
    // port d'écriture IEditeurLieux, de sorte que le lieu cesse d'être énuméré.
    [Fact]
    public void Should_Faire_disparaitre_le_lieu_de_l_enumeration_When_le_parent_supprime_un_lieu()
    {
        var referentiel = new FakeReferentielLieux().AvecLieu(Nounou);
        var handler = new SupprimerLieuHandler(referentiel);

        var resultat = handler.Handle(new SupprimerLieuCommand(Nounou));
