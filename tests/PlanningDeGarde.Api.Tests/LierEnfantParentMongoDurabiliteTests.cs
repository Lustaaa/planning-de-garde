...
/// parent-acteur via le handler câblé sur le store durable <see cref="ReferentielEnfantsMongo"/> ;
/// le redémarrage du serveur est matérialisé par une <b>nouvelle instance de store</b> sur la
/// <b>même base Mongo</b> persistée : après redémarrage, l'enfant relu doit toujours porter le parent
/// dans <see cref="EnfantFoyer.ParentsLies"/>, avec son identifiant stable inchangé — preuve que le
/// lien a atteint le store durable (write-through), pas un cache de session. Un enfant non lié reste
/// valide avec une liste de parents vide (lien optionnel, 0 parent accepté).
///
...
public sealed class LierEnfantParentMongoDurabiliteTests : IDisposable
{
    ...
    public void Acceptation_Should_Relire_le_parent_lie_sur_le_meme_id_apres_redemarrage_When_l_enfant_a_ete_lie_sur_le_store_Mongo_reel()
    {
        ...
        string leaId, tomId;
        const string papaId = "acteur-papa";
        {
            ...
            var lier = new LierEnfantParentHandler(store1);
