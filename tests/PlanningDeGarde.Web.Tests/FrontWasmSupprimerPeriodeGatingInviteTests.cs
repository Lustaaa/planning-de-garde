...
/// « Supprimer une période » du menu clic-case est gardée par le déclencheur d'écriture <b>mutualisé</b>
...
/// <b>Parent</b> ouvre bien le menu ET y voit l'entrée « Supprimer une période » (le déclencheur n'est pas
/// cassé pour tous), puis on bascule en <b>Invité</b> et on prouve que le clic n'ouvre ni menu ni entrée ni
/// dialog de suppression, et que la période reste inchangée dans le store distant.
/// </summary>
public sealed class FrontWasmSupprimerPeriodeGatingInviteTests : TestContext
{
    private static readonly DateTime Mardi_16_06_2026 = new(2026, 6, 16);

    [Fact]
    public void Should_Ne_proposer_aucune_entree_ni_dialog_de_suppression_et_laisser_la_periode_inchangee_When_un_invite_en_consultation_seule_clique_une_case()
    {
        ...
        // « Supprimer une période » (le déclencheur n'est pas cassé pour tous). Idempotent sous WaitForAssertion.
        ...
        Assert.Empty(grille.FindAll("[data-testid='menu-actions-case']"));
        Assert.Empty(grille.FindAll("[data-testid='action-supprimer-periode']"));
