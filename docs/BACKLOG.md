...
*(Aucun sprint en cours.)* Dernier livré = **s38 `graphe-foyer-enfant-racine`** (**6ᵉ incrément épic Refonte
...
>
> **Candidats goal prochain `/planning`** (**graphe enfant-racine + R2 familles recomposées lecture résolus
...
> (0) **[P1, dette flake — CANDIDAT DE TÊTE, 5ᵉ montée chiffrée au gate s36, mitigation confirmée s37]**
> **rétrofit du garde *TempsReel* SignalR** (sérialiser les assemblies I/O en collections xUnit non
> parallèles) — le mode gate `-Serial` (test.ps1, s36, **désormais étape gate par défaut dans `/sprint` —
> rétro s37**) a de nouveau donné 682/682 en un run au gate s37 (le parallèle rougissant sur le flake), mais
> la dette de test reste à solder à la cause avant tout nouveau client SignalR ;
...
> (5) **commandes inverses actif/admin** (dé-désignation admin + désactivation de compte — le toggle
...
| **Familles recomposées** (enfants de parents différents, même planning) — **VISIBLES en LECTURE s38** (vue graphe enfant-racine : deux racines, parent partagé sous chaque enfant, reflet fidèle) ; reste ouvert : **vue planning centrée couple** (recomposé) + **R3 « exactement 2 parents »** | 🟡 | Palier 5-6 | spec règle 2 · retours s07 · **partiel s38** |
| ~~**Graphe foyer enfant-racine** (vue d'accueil Config foyer)~~ **livré s38 EN LECTURE** : query PURE `GrapheFoyerQuery` (par enfant → parents liés + rôle-du-lien, deux adaptateurs InMemory+Mongo) + **vue lecture seule** (enfant en racine « nom (rôle) », familles recomposées visibles, store vide = message neutre, Parent-gated, convergence SignalR par **reprojection client**). **Reste ouvert (graphe ÉTENDU)** : grands-parents, parents liés entre eux via leurs enfants, lien enfant↔activité dans le graphe, **édition depuis le graphe** | 🟡 | Palier 5-6 | retours s07 · spec règles 2-3 · **partiel s38** |
| **Deux parents (toujours exactement 2 ; le 1er saisit l'autre)** — **R3 non traité s38** (le graphe RESTITUE 0/1/2 parents tels quels, n'impose aucune complétude du couple) | ⬜ | Palier 5 | retours s01 · spec règle 3 · hors scope s38 |
