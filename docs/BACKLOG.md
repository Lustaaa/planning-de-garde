...
|-------:|-------|-------------------|:------:|
| — | *(aucun sprint en cours ; s24 `auth-utilisable-activation-et-page-login` livré et clôturé — gate G3 validé PO, 11/11, 380/380. **Auth utilisable de bout en bout LIVRÉE** : activation `Inactif→Actif` (prérequis P0 **LEVÉ**) + page de connexion dédiée `/connexion` (landing) + menu utilisateur. La boucle auth (s22 fondation + s23 session + s24 activation/login) est désormais **fonctionnelle E2E**. Section retours PO **NON vide** (consommés ci-dessous) : **1 besoin — mot de passe** (facteur d'auth par mot de passe, à arbitrer contre OAuth) ; **1 rappel — OAuth Google/Microsoft/Apple** (déjà 2b) ; **2 bugs/besoins** — *rôle ≠ acteur du compte connecté* (« connecté en Mamie, j'ai le rôle Parent ») et *toutes les pages accessibles sans être loggé* (protection d'accès aux routes) ; **plus tard** — envoi de mail d'activation (description à préciser). **Prochains gros items** : rétrofit flake P1 (candidat de TÊTE s25, cf. ci-dessous), auth 2b OAuth, création de compte libre-service, récup mot de passe, cohérence config→planning, sprint design)* | auth utilisable E2E : activation + page login (s24) ; connexion locale + session (s23) ; fondation identité (s22) ; modèle de rôles (s21) | — |
...
| **+1 (P1 — URGENT PO, prochain gros item)** | **Auth tranche 2b — OAuth externe** : **OAuth Google / Apple / Microsoft** (3 intégrations externes, secrets, callbacks, providers réels) branchées derrière la session serveur livrée s23. Non testables en runtime local → **arbitrer la stratégie de preuve au make-gherkin** (rempart acceptation runtime). S'appuie sur la session s23 (socle prêt à recevoir un provider) | É10, É5 | **URGENT PO** ; la connexion locale (s23) est posée, il reste les providers externes ; à cadrer/découper au make-gherkin (OAuth non testable en runtime local) |
| **+1 (P1 — retours PO s23/s24) — UX auth : volets restants** | **Page de connexion dédiée** + **accès config foyer via menu utilisateur** : **LIVRÉS s24** (route `/connexion` landing, bandeau inline retiré, `MenuUtilisateur`). **Restent** : **création de compte en libre-service** (« si on en a pas encore ») + **récupération de mot de passe par email** via un **adaptateur de droite d'envoi de mail**. Le PO redemande s24 un **mot de passe** → ces volets introduisent un facteur d'auth **par mot de passe** distinct de l'email-only livré, **à arbitrer contre OAuth 2b** au make-gherkin. *(Plus tard, non spécifié : envoi de mail d'activation — description à préciser.)* | É10, É5, É2 | Retours PO s23 (parcours) + s24 (mot de passe) ; la page login est livrée, restent l'inscription libre-service et le facteur mot de passe (adaptateur mail) |
...
| **+1 (P1 — CANDIDAT DE TÊTE INCONTOURNABLE s25, 4ᵉ montée de sévérité s24)** | **Rétrofit complet du garde *TempsReel* SignalR** — cibler la **convergence SignalR multi-clients** (distincte de la course d'énumération déjà gardée s13). **VRAI flake résiduel = intermittent sous charge parallèle** (`FrontWasm*TempsReel*` sur tests SignalR multi-clients, vert isolé) — **dette de test réelle**, aucune cause `src/`. **4ᵉ montée de sévérité s24** : l'auth ajoute encore des clients SignalR → **jusqu'à 6 flakes simultanés** sous charge `Web.Tests`, **la suite exige souvent un 2ᵉ run** pour être verte. Le triage durci (rétro s21) **tient toujours** (chaque rouge *TempsReel* re-testé en isolation → vert → classé flake, jamais étiqueté à tort ni investigué `src/`) — **pas une friction méthode**, mais la **dette menace sérieusement la fiabilité du gate de non-régression**. **DÉCISION CLÔTURE s24** : ce rétrofit devient le **candidat de TÊTE du sprint 25** — **à traiter AVANT tout nouveau feature ajoutant des clients SignalR** (dont auth 2b/libre-service). | É3 | Déverrouille l'édition concurrente sans driver une fondation temps-réel instable ; **le gate 380/380 exige déjà souvent 2 runs** — chaque client SignalR neuf aggrave ; priorité montée en **tête** (arbitrer G2 en 1er) |
| **+2 (P1 — retours PO s24) — protection d'accès aux routes** | **Garde d'accès non authentifié** — « les pages sont toutes accessibles même sans être loggé ». La landing `/connexion` (s24) existe mais les routes (planning, config) **ne sont pas gardées** : un utilisateur non connecté y accède directement. Sujet : guard / redirection des routes protégées vers `/connexion` tant qu'aucune session, cohérent avec la session serveur s23 | É10 | Retour PO direct s24 ; faille d'accès une fois l'auth utilisable ; à cadrer au make-gherkin |
| **+3 (P1 — retours PO s24) — bug rôle ≠ acteur connecté** | **Rôle affiché incohérent avec le compte connecté** — « quand je me connecte avec Mamie, j'ai le rôle Parent ». L'identité effective / le rôle ne suit pas toujours l'acteur du compte connecté (recoupe la cohérence config → planning s21). Sujet : investiguer le câblage session → identité effective → rôle, corriger | É10, É2 | Retour PO direct s24 (bug) ; l'auth est utilisable mais le rôle effectif est faux dans ce cas |
...
| **Création de compte en libre-service** (« si on en a pas encore ») — s'inscrire soi-même, distinct de la création d'acteur par le parent (s22) | ⬜ | Palier 13 (P1) | retours s23 |
| **Récupération de mot de passe par email** via un **adaptateur de droite d'envoi de mail** — introduit un facteur d'auth **par mot de passe** (distinct de l'email-only et de l'OAuth 2b, à arbitrer) ; PO redemande un mot de passe s24 | ⬜ | Palier 13 (P1) | retours s23 · s24 |
| **Protection d'accès aux routes** (garde non authentifié) — « les pages sont toutes accessibles même sans être loggé » : redirection des routes protégées vers `/connexion` tant qu'aucune session | ⬜ | Palier 13 (P1) | retours s24 |
| **Bug — rôle ≠ acteur du compte connecté** (« connecté en Mamie, j'ai le rôle Parent ») : l'identité effective / le rôle ne suit pas l'acteur du compte connecté (recoupe cohérence config → planning s21) | ⬜ | à séquencer (P1) | retours s24 |
