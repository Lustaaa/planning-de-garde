...
|-------:|-------|-------------------|:------:|
| — | *(aucun sprint en cours ; s22 `auth-fondation-identite` livré et clôturé — gate G3 validé PO, 9/9, 351/351. Section retours PO **VIDE** : PO a validé sans nouveau retour. **Auth tranche 1 (fondation identité + invariant admin=parent) LIVRÉE** ; **auth tranche 2 = prochain gros item URGENT** — voir « Prochains sprints envisagés »)* | auth tr.1 : fondation identité compte↔acteur + invariant admin=parent (s22) ; modèle de rôles (s21) ; config en onglets (s20) | — |
...
| **+1 (P1 — URGENT PO, prochain gros item)** | **Auth tranche 2 — connexion réelle** : **page de connexion custom** + **OAuth Google / Apple / Microsoft** (3 intégrations externes, secrets, callbacks) + **sessions HTTP réelles / logout** + **couplage « acteur par défaut config = utilisateur connecté »** (le compte de l'utilisateur connecté → acteur par défaut, dépend de la session). S'appuie sur la fondation identité s22 (compte↔acteur 1-1). L'**activation** (prise en main de compte, statut Inactif→Actif) et les **droits par rôle** restent séquencés après (palier 13) | É10, É2, É5 | **URGENT PO — prochain gros item** ; la fondation identité (s22) est posée, il reste le palier lourd de la connexion réelle (landing, OAuth, sessions) ; débloque « acteur par défaut = moi » et ouvre la voie aux droits par rôle. À cadrer/découper au make-gherkin (OAuth non testable en runtime local — arbitrer stratégie de preuve) |
...
| **URGENT — Auth tranche 2 : page de connexion custom + OAuth Google / Apple / Microsoft + sessions HTTP / logout** (comptes utilisateurs réels, connexion réelle ; s'appuie sur la fondation identité s22) | ⬜ | Palier 13 (P1, **prochain gros item**) | retours s21 (**URGENT**) |
| **URGENT — Acteur par défaut config = utilisateur connecté** (défaut de sélection = l'utilisateur authentifié ; dépend d'une session → auth tranche 2. Relation identité↔acteur **posée s22**, couplage trivial une fois la session en place) | ⬜ | Palier 13 (P1, tr.2) | retours s21 (**URGENT**) |
