...
|-------:|-------|-------------------|:------:|
| — | *(aucun sprint en cours ; s20 `config-foyer-onglets` livré et clôturé — gate G3 validé PO, 7/7, 288/288, aucun retour produit nouveau)* | 9 + 14 livrés (s15) ; période suppr./édit. (s16/s17) ; suppr. slot (s18) ; acteurs réels partout (s19) ; config en onglets + convergence dernier sélecteur (s20) | — |
...
| **+2 (P1 — priorité montée s20)** | **Rétrofit complet du garde déterministe *TempsReel* SignalR** — cibler la **convergence SignalR multi-clients** (distincte de la course d'énumération déjà gardée s13) ; **visibilité en hausse continue sous charge**, **2ᵉ montée de sévérité confirmée** : `FrontWasmInvitePlageIndisponibleTempsReel` rouge **2/3 runs full-suite au s18**, débordement hors `*TempsReel*` **~2/10 au s19**, et **au s20 l'écran config devient un 2ᵉ client SignalR** dans les tests grille+config → le flake de convergence multi-clients monte à **~1-2 tests par run complet** (toujours un seul *TempsReel* à la fois, **vert au re-run ciblé**, tout vert hors *TempsReel*, **aucune cause dans `src/`**) — **dette de test** qui menace le gate 288/288 : **chaque sprint ajoutant un client SignalR aggrave le flake**, prérequis de l'édition concurrente | É3 | Déverrouille l'édition concurrente sans driver une fondation temps-réel instable ; **priorité montée à P1** — la sévérité croît à chaque nouveau client SignalR et rapproche le gate de non-régression du rouge (à arbitrer G2 contre les retours produit restants : modèle de rôles, transfert bicolore) |
