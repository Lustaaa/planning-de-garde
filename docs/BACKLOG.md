...
|-------:|-------|-------------------|:------:|
| — | *(aucun sprint en cours ; s18 `supprimer-slot` livré et clôturé — gate G3 validé PO, 10/10, 270/270, aucun retour produit nouveau)* | 9 + 14 livrés (s15) ; suppression (s16) + édition (s17) de période ; suppression de slot (s18) | — |
...
| +2 | **Rétrofit complet du garde déterministe *TempsReel* SignalR** — cibler la **convergence SignalR multi-clients** (distincte de la course d'énumération déjà gardée s13) ; **visibilité en hausse sous charge** : `FrontWasmInvitePlageIndisponibleTempsReel` rouge **2/3 runs full-suite au s18** (Sc.7, vert isolé + re-run) — bien au-delà du 1/30 historique — **dette de test** qui menace désormais le gate de non-régression, prérequis de l'édition concurrente | É3 | Déverrouille l'édition concurrente sans driver une fondation temps-réel instable ; sa fréquence montante fragilise le gate vert (signal de priorisation) |
