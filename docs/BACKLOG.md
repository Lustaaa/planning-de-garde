...
*(Aucun sprint en cours.)* Dernier livré = **s32 `refonte-config-acteurs-crayon-modal`** (**1er
incrément vertical de l'épic Refonte Config foyer**) : onglet Acteurs migré du patron **édition
inline** au patron **tableau lecture seule + crayon → modal** (états actif/admin en pastille lecture ;
modal éditant nom/couleur/rôle via les commandes CRUD **existantes**, id stable non éditable ;
« Ajouter » = même modal vide → id stable neuf ; refus domaine → modal reste **ouverte** + motif +
saisie conservés ; **Parent-gated**, Invité = lecture seule sans crayon ni « Ajouter » ; **temps réel
SignalR** convergent sans reload). Sprint **pur @ihm** (0 @back, aucun handler/query neuf) ; **swap
atomique inline→modal** Sc.1-4 en un commit (~34 fichiers d'acceptation migrés), erreur/gating/SignalR
en incréments propres ; 7/7 ✅, suite **569/569**, gate G3 validé PO. Prochain = `/planning`.
...
> **Candidats goal prochain `/planning`** : (1) **arbitrage inline vs modal** ci-dessus (tension s32) ;
> (2) **2ᵉ incrément de l'épic Refonte Config foyer** — **harmonisation Rôles / Cycle / Enfants** sur le
> même patron tableau+crayon→modal (dont **cycles déclarés qui n'apparaissent pas** dans la config
> acteurs) + **champ neuf « adresse postale » de l'acteur** ; (3) **suppression d'un slot récurrent depuis
> l'IHM** (retour PO gate s31, affordance IHM manquante + nuance occurrence unique vs série). **D2**
> (config slots récurrents en Config foyer + récurrence **multi-jours**) reste dette ouverte séparée.
> **Google OAuth réel** (P0) reste en tête.
...
| **Lien enfant↔parent** (« comment lier les enfants au parent ? ») — à traiter **dans la refonte des acteurs** (épic Refonte Config foyer, +1 P0) ; vue d'accueil config = **graph enfant en racine** | ⬜ | épic Refonte Config foyer | retours s29 (PO) · brief config foyer |
| **Harmoniser l'onglet Enfants** (livré s30) avec le reste de la config (tableau lecture + modal, patron commun) — dans l'épic Refonte Config foyer | ⬜ | épic Refonte Config foyer | retours s29 (PO) · brief config foyer |
...
| **Refonte Config foyer — patron tableau lecture seule + crayon → modal** — **1er incrément ACTEURS livré s32** (tableau lecture seule + crayon→modal éditant les champs **existants** nom/couleur/rôle via CRUD existant ; ajouter = modal vide ; refus→modal ouverte ; Parent-gated + SignalR ; inline retiré). **Reste** : état actif/admin en **toggle** *dans* la modal (auj. pastille lecture), **adresse** + **palette couleur** (champs neufs), harmonisation **rôles & cycle**, arbitrage inline vs modal. Brief : [`docs/briefs/refonte-configuration-foyer.md`](briefs/refonte-configuration-foyer.md). | 🟡 | épic Refonte Config foyer (1er incr. s32) | retours s28/s29 (PO) · brief · **1er incr. s32** |
...
| **Adresse de résidence de l'acteur** (champ de modèle neuf, exposé dans la modal d'édition acteur) — **2ᵉ incrément** épic Refonte Config foyer (re-signalé gate s32) | ⬜ | épic Refonte Config foyer (2ᵉ incr.) | brief config foyer (PO) · re-signalé s32 |
