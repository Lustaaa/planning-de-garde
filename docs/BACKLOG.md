...
|-------:|-------|-------------------|:------:|
| — | *(aucun sprint en cours ; s19 `acteurs-reels-partout` livré et clôturé — gate G3 validé PO, 7/7, 281/281, aucun retour produit nouveau)* | 9 + 14 livrés (s15) ; période suppr./édit. (s16/s17) ; suppr. slot (s18) ; acteurs réels partout (s19) | — |
...
| +2 | **Rétrofit complet du garde déterministe *TempsReel* SignalR** — cibler la **convergence SignalR multi-clients** (distincte de la course d'énumération déjà gardée s13) ; **visibilité en hausse sous charge** : `FrontWasmInvitePlageIndisponibleTempsReel` rouge **2/3 runs full-suite au s18** (vert isolé + re-run) — bien au-delà du 1/30 historique — et **au s19 le flake déborde hors `*TempsReel*`** (~2/10 runs, ex. `FrontWasmDefinirTransfertGatingInvite`) sous la charge multi-clients du test Sc.7 — **dette de test** qui menace désormais le gate de non-régression, prérequis de l'édition concurrente | É3 | Déverrouille l'édition concurrente sans driver une fondation temps-réel instable ; sa fréquence montante **et son débordement hors *TempsReel*** fragilisent le gate vert (signal de priorisation renforcé) |
...
- **Sélecteur d'édition de l'écran config encore sur `Foyer.ActeursEditables` (É2, hors-scope s19)** — au s19, sélecteurs de dialogs + grille + légende ont convergé sur le **store vivant unifié** des acteurs déclarés (`IEnumerationActeursFoyer`, id stable), mais le **sélecteur d'édition de l'écran config** lit **encore** `Foyer.ActeursEditables` : noms **réels, zéro fantôme** (pas de régression d'usage), **mais pas la source unifiée**. **Dette de cohérence** (deux chemins de lecture du référentiel acteurs) — candidat backlog : converger le sélecteur config sur `IEnumerationActeursFoyer` pour un seul chemin de lecture. Signalé par la dev-team à la clôture s19.
