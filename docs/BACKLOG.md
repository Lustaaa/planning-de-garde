...
*(Aucun sprint en cours.)* Dernier livré = **s36 `eligibilite-parent-type-acteur`** (**révision
...
>
> **Candidats goal prochain `/planning`** (Éligibilité parent **résolue s36** ; Activités harmonisées +
> renommage + lien enfant↔activité **livrés s35** ; Enfants + lien enfant↔parent **livrés s34** ; Acteurs
> 2ᵉ incr. + Rôles/Cycle **livrés s33**) :
> (0) **[P1, dette flake — 5ᵉ montée chiffrée au gate s36]** **rétrofit du garde *TempsReel* SignalR**
> (sérialiser les assemblies I/O en collections xUnit non parallèles) — le mode gate `-Serial` (test.ps1,
> s36) mitige le coût immédiat mais la dette de test reste à solder avant tout nouveau client SignalR ;
> (1) **familles recomposées R2/R3** (« exactement 2 parents » + graphe enfant-racine, **vue d'accueil =
> graph enfant en racine**) + **champ père/mère distinct** sur le lien enfant↔parent (distinction par le NOM
> aujourd'hui, hors scope s36) — suite naturelle du lien s34/s36 ;
...
| **Champ père/mère distinct sur le lien enfant↔parent** (distinction des 2 parents par le NOM aujourd'hui — hors scope s36) : porter un attribut « rôle-du-lien » (père/mère ou libre) pour distinguer les 2 parents liés | ⬜ | épic Refonte Config foyer | hors scope s36 · lien s34/s36 |
