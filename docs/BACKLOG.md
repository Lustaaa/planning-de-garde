...
*(Aucun sprint en cours.)* Dernier livré = **s35 `lieux-vers-activites`** (**4ᵉ incrément vertical
...
> **⚠️ PRIORITAIRE — retour PO gate s35 : « Papa/Maman sont OBLIGATOIREMENT des rôles Parent, donc liables à
> un enfant ».** Le lien enfant↔parent s34 n'admet un acteur comme parent QUE s'il porte un **rôle du
> référentiel dont le libellé vaut LITTÉRALEMENT « Parent »** (`LierEnfantParentHandler` L44-58 ET IHM
> `ConfigurationFoyer.razor.cs` `ActeursParents()` L393-395) — il **IGNORE `TypeActeur.Parent`** (l'enum
> `Parent|Admin|Autre` qui sert au gating d'impersonation, seed Alice/Bruno = Parent). Conséquence
> contre-intuitive : un Papa/une Maman ne sont liables qu'en leur créant un rôle nommé exactement
> « Parent », et deux parents ainsi liés affichent le même libellé « Parent » (aucune distinction
> Papa/Maman). **RÉVISION D'INVARIANT** (qu'est-ce qui qualifie un acteur comme « parent » liable) —
> **NON absorbable dans s35** (= Lieux→Activités, ne touche pas cette règle). **À trancher en G2 au
> prochain `/planning`** (cf. Épic 1). Options esquissées : **(A)** éligibilité = **`TypeActeur.Parent`**
> (découple du libellé de rôle ; Papa/Maman deviennent des rôles d'affichage libres) ; **(B)** flag
> **« rôle parent »** explicite sur `RoleFoyer` (plusieurs libellés — Papa, Maman — marqués « parent »)
> au lieu du match littéral « Parent » ; **(C)** **unifier** les deux notions en une source de vérité
> unique de « qui est parent ». À arbitrer avec l'impact sur le gating (R8) et le seed démo (ni Papa/Maman
> ni rôle « Parent » ni enfant seedés aujourd'hui).
>
> **Candidats goal prochain `/planning`** (Activités harmonisées + renommage + lien enfant↔activité **livrés
> s35** ; Enfants + lien enfant↔parent **livrés s34** ; Acteurs 2ᵉ incr. + Rôles/Cycle **livrés s33**) :
> (0) **[PRIORITAIRE, retour PO gate s35]** **révision de l'éligibilité « parent » du lien enfant↔parent**
> — un acteur `TypeActeur.Parent` (Papa/Maman) doit être liable **sans** devoir créer un rôle nommé
> littéralement « Parent » (cf. bloc ci-dessus + Épic 1 ligne dédiée) ;
> (1) **prolonger les Activités (post-s35)** — **liste de slots par activité** (récurrents/non ; une activité
...
> (1bis) **familles recomposées R2/R3** (contrainte « exactement 2 parents », graphe foyer, **vue d'accueil =
> graph enfant en racine**) — suite naturelle du lien enfant↔parent s34 ;
...
| **⚠️ [PRIORITAIRE, retour PO gate s35] Révision de l'éligibilité « parent » du lien enfant↔parent** : le lien s34 (`LierEnfantParentHandler` L44-58 + IHM `ActeursParents()` L393-395) n'admet un acteur comme parent que s'il porte un **rôle du référentiel de libellé LITTÉRAL « Parent »**, et **ignore `TypeActeur.Parent`**. Le PO exige qu'un **Papa/une Maman (rôles Parent par nature)** soient liables **sans** devoir créer un rôle nommé exactement « Parent ». **Révision d'invariant** (définir ce qui qualifie un acteur « parent » liable). Options : **(A)** éligibilité = `TypeActeur.Parent` (Papa/Maman = rôles d'affichage libres) ; **(B)** flag « rôle parent » explicite sur `RoleFoyer` (Papa, Maman marqués parent) ; **(C)** unifier les deux notions en une source unique. Impact à cadrer : gating impersonation (R8), seed démo (ni Papa/Maman ni rôle « Parent » aujourd'hui), affichage distinct Papa/Maman. **NON absorbable dans s35** (Lieux→Activités). **À TRANCHER en G2 prochain `/planning`**. | ⬜ | **à arbitrer G2** | retours gate s35 (PO) · lien s34 |
...
| **Refonte Config foyer — patron tableau lecture seule + crayon → modal** — **Acteurs 1er incr. livré s32**, **Acteurs 2ᵉ incr. + Rôles & Cycle livrés s33** (toggle actif/admin sens ON + verrou, adresse, palette picker ; Rôles & Cycle au patron ; **tous les cycles déclarés visibles** ; fermeture Échap des modals), **Enfants harmonisés + lien enfant↔parent livrés s34** (patron tableau+crayon→modal, swap inline→modal, colonne « Parents liés » + sélecteur, lien 1..2 parents), **Activités harmonisées + renommage Lieux→Activités + lien enfant↔activité livrés s35** (4ᵉ incr. ; swap inline→modal, renommage HTTP au SWAP, champ adresse, colonne « Enfants liés » + sélecteur d'enfants). **Épic : Acteurs / Rôles / Cycle / Enfants / Activités TOUS harmonisés.** **Reste** : **arbitrage inline vs modal**, **commandes inverses actif/admin**. Brief : [`docs/briefs/refonte-configuration-foyer.md`](briefs/refonte-configuration-foyer.md). | 🟡 | épic Refonte Config foyer (Acteurs+Rôles+Cycle+Enfants+Activités faits) | retours s28/s29 (PO) · brief · **s32+s33+s34+s35** |
