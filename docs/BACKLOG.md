...
| **+1 (P0 — ÉPIC : Refonte de la Configuration du foyer ; retour PO structuré s28→s29, réserve LEVÉE)** | **Refonte de la Configuration du foyer** — brief PO complet : [`docs/briefs/refonte-configuration-foyer.md`](briefs/refonte-configuration-foyer.md). Harmoniser toutes les sections config sur un même patron **tableau lecture seule + crayon → modal**. **(A) Acteurs** : tableau lecture seule (nom, email, rôle, **état en pastille** actif/admin) ; colonne action **crayon → modal** éditant **tous** les champs (nom, email, rôle, **état en toggle** actif/admin, **adresse de résidence** [champ neuf], **couleur avec vraie palette** [picker, cf. dette set couleurs]) ; bouton **ajouter → même modal vide**. Remplace l'édition **inline** actuelle. **(B) Rôles** : harmonisés (tableau lecture + modal). **(C) Cycle** : harmonisé (tableau + modal ; a minima tableau des cycles settés/actifs). **(D) Lieux → « Activités »** : le PO repense le lieu comme une **activité liée à l'enfant** (nom, **adresse**, **liste de slots** avec flag récurrent/non ; plusieurs enfants peuvent partager la même activité) — **renommage sémantique + lien enfant↔activité** (à cadrer : impact sur le référentiel de lieux s27 + validation de pose). **(E) Enfants** : **harmoniser** l'onglet Enfants (livré s30) avec le reste ; **vue d'accueil lecture seule = graph avec l'enfant en racine** ; **lien enfant↔parent** (« comment lier les enfants au parent ? À faire pendant la refonte des acteurs »). **Bonus (2ᵉ temps)** : multi-enfants configurables, vue planning centrée sur la garde des enfants d'un couple, **vue foyer recomposé** + proposition de config dédiée. | É1, É2, É7, É6 | Retour PO structuré (annoncé /planning s29, capté clôture s30) ; gros épic à **découper au /planning** en incréments verticaux (acteurs crayon/modal en tête, cohérent dialogs s11-s12 ; enfants/activités/graph ensuite) ; introduit des champs neufs (adresse, palette couleur) et un lien enfant↔parent |
...
| **Refonte Config foyer — patron tableau lecture seule + crayon → modal** (acteurs : modal éditant nom/email/rôle/état-toggle actif-admin/**adresse de résidence** [champ neuf]/**couleur vraie palette** [picker] ; ajouter = modal vide ; rôles & cycle harmonisés). Remplace l'édition inline. Brief : [`docs/briefs/refonte-configuration-foyer.md`](briefs/refonte-configuration-foyer.md). Épic candidat +1 (P0), à découper au /planning. | ⬜ | épic Refonte Config foyer | retours s28/s29 (PO) · brief config foyer |
| **Adresse de résidence de l'acteur** (champ de modèle neuf, exposé dans la modal d'édition acteur) | ⬜ | épic Refonte Config foyer | brief config foyer (PO) |
