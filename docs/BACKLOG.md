...
|-------:|-------|-------------------|:------:|
| — | *(aucun sprint en cours ; s23 `auth-session-locale-acteur-par-defaut` livré et clôturé — gate G3 validé PO, 9/9, 367/367. **Auth tranche 2a (connexion locale + session serveur + acteur-par-défaut = moi) LIVRÉE**. Section retours PO **NON vide** (4 items, consommés ci-dessous) : le PO juge l'UX peu naturelle (bandeau login sur l'écran planning « n'a aucun sens ») → veut **page de connexion dédiée = page par défaut** + **création de compte** libre-service + **récupération mot de passe par email (adaptateur de droite mail)** + **accès config foyer depuis un menu utilisateur** + question « comment activer le compte ? ». **SIGNAL BLOQUANT** : l'**activation `Inactif→Actif` est un prérequis d'usabilité** — aucune connexion ne réussit sur données réelles aujourd'hui. **Auth tranche 2b (OAuth) + activation = prochains gros items** — voir « Prochains sprints envisagés »)* | auth tr.2a : connexion locale + session + acteur-par-défaut = moi (s23) ; auth tr.1 fondation identité (s22) ; modèle de rôles (s21) | — |
...
| **+1 (P0 — PRÉREQUIS D'USABILITÉ BLOQUANT)** | **Activation de compte `Inactif → Actif` (prise en main)** — les comptes naissent `Inactif` (s22) et **aucun chemin d'activation** n'existe, donc **aucune connexion (s23) ne réussit sur données réelles** aujourd'hui. Sujet : un chemin d'activation (prise en main par l'utilisateur réel, ou bascule Actif par l'admin/parent), statut passant `Inactif→Actif`, gardes. **Débloque l'usage réel de toute l'auth livrée (s22 + s23)** | É10, É2 | **À prioriser haut — probablement AVANT ou AVEC la tranche 2b** ; sans lui l'auth est fonctionnellement inerte en prod. Testable en runtime local (pas d'OAuth). Recoupe « prise en main de son compte » (É10, retours s08) |
...
| **+1 (P1 — retours PO s23) — UX auth naturelle** | **Refonte du parcours de connexion** : **page de connexion dédiée = page par défaut** de l'app (le bandeau login sur l'écran planning « n'a aucun sens » selon le PO) + **création de compte** en libre-service (« si on en a pas encore ») + **récupération de mot de passe par email via un adaptateur de droite d'envoi de mail** + **accès à la config foyer depuis un menu utilisateur** une fois connecté. Recoupe la landing page (É10) et la refonte visuelle (É5) | É10, É5, É2 | Retours PO directs s23 ; l'auth locale (s23) est livrée mais son UX est jugée peu naturelle ; à cadrer/découper — certains volets (mot de passe, adaptateur mail) introduisent un facteur d'auth **par mot de passe** distinct de l'email-only s23, à arbitrer contre OAuth 2b |
...
> révèle le **prérequis d'usabilité bloquant** (activation `Inactif→Actif`, monté **P0**, ci-dessus et
...
| **Page de connexion dédiée = page par défaut de l'app** (le bandeau login sur l'écran planning « n'a aucun sens » — PO s23) + landing identifiant le foyer et invitant à s'authentifier | ⬜ | Palier 13 (P1) | retours s01 (#2) · **retours s23** · spec p8 |
...
| **Récupération de mot de passe par email** via un **adaptateur de droite d'envoi de mail** — introduit un facteur d'auth **par mot de passe** (distinct de l'email-only s23 et de l'OAuth 2b, à arbitrer) | ⬜ | Palier 13 (P1) | retours s23 |
| **Accès à la config foyer depuis un menu utilisateur** une fois connecté | ⬜ | Palier 13 (P1) | retours s23 |
...
| **PRÉREQUIS D'USABILITÉ (P0) — activation de compte `Inactif → Actif`** : les comptes naissent Inactif (s22), aucun chemin d'activation → **aucune connexion (s23) ne réussit sur données réelles** aujourd'hui. Débloque l'usage réel de l'auth livrée. Recoupe « prise en main de son compte » ci-dessous | ⬜ | Palier 13 (**P0, avant/avec 2b**) | retours s23 (question PO « comment activer le compte ? ») |
