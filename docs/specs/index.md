...
| Acteurs & configuration du foyer | [`acteurs-et-config-foyer.md`](acteurs-et-config-foyer.md) | Paliers 4/5/8/13 : édition, ajout, persistance Mongo, CRUD complet, gating, impersonation bornée lecture ; **référentiel de lieux éditable + persisté** (s27, pilote validation de pose ET sélecteurs des dialogs) ; **référentiel d'enfants éditable + persisté** (s30, enfant hissé en agrégat de 1er rang, onglet Enfants + sélecteur explicite, validation d'existence à la pose, R1) ; **auth / login complet** — fondation identité, connexion, activation, protection routes, mot de passe, libre-service, récupération, OAuth (R1-9 + R11-R14) ; **câblage réel reset E2E + login mot de passe + amorçage démo convergent** (s28, R14bis) ; **persistance/restauration de session côté client (survie au F5) + bouton œil** (s31, R14ter) ; **onglet Acteurs refondu en tableau lecture seule + crayon → modal** (s32, 1er incrément Refonte Config foyer, CRUD existant, Parent-gated, SignalR). |
