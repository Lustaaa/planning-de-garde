...
| Saisie visible, grille lisible & thème | [`saisie-et-grille.md`](saisie-et-grille.md) | Paliers 2-3 : saisie visible, lisibilité (couleur + nom + légende), thème (règles 18-22) ; **rendu bicolore du transfert sur la pastille de date** — saisi (s29) **et AUTO-dérivé** (s31, présentation) ; **carte du jour « Qui récupère ce soir »** en tête du planning (s42, LECTURE seule, query PURE composant la résolution/slots/transfert, reprojection SignalR client, limitation « visible pour le jour courant dans la fenêtre chargée ») ; **panneau « À venir »** sous la carte (s43, LECTURE seule, query PURE `AVenirQuery` composant `GrilleAgendaQuery`, prochains jours de la fenêtre chargée, repli neutre, reprojection SignalR, même limitation « fenêtre chargée »). |
...
| Écriture en contexte — dialogs | [`ecriture-en-contexte.md`](ecriture-en-contexte.md) | Palier 7 : menu clic-case, trois dialogs, issues succès/échec/chevauchement, gating (règles 14/16/17/24/25/28) ; **slot récurrent hebdo simple** via la dialog « Poser un slot » unifiée (s29) ; **slot récurrent conditionné à la garde** (toggle « seulement les jours où l'enfant est chez moi », s31). |
