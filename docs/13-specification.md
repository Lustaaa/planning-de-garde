...
> Version 13 · consolide la v12 + docs/sprints/12-transfert-en-contexte/99-sprint12-besoins-fin-itération.md. Remplace la v12, qui reste figée en historique.
...
L'**appropriation des acteurs se poursuit** : avec l'ajout et l'édition acquis,
le **cycle de vie des acteurs** se complète par la **suppression** (Delete),
prochain sujet d'usage. Supprimer un acteur du foyer **neutralise** ses cases
orphelines (la case retombe sur le fond, ou le neutre), sans réaffectation
automatique — c'est l'incrément suivant.
...
> « récurrence des périodes » et « écriture en contexte (dialogs, **transfert
> inclus**) » sont **livrés**. C'est cet arbitre qui tire la **suppression
> d'acteur** (usage) **devant** le calendrier navigable.
...
> remonter devant l'usage à sa suite. La **suppression d'acteur** (sujet courant)
> opère sur ce **même store durable** → acceptation runtime obligatoire.
...
> de le confirmer : déplacer la saisie en dialogs — **transfert compris** — n'a
> tiré **aucune** persistance (slots / périodes / transferts restent en mémoire).
> La **suppression d'acteur** opère, elle, sur la config foyer **déjà durable**
> (palier 5) : elle ne crée pas de cliquet, elle exerce une persistance acquise.
...
> Définir un transfert) plutôt qu'en bloc. Il guide aussi le palier courant :
> **CRUD acteurs** se découpe en **suppression d'abord**, **amorce
> d'impersonation ensuite**. Il reste le **garde-fou de secours** des sujets pris
> en bloc.
...
> **Prochain sujet : CRUD acteurs — tranche suppression.** L'écriture en contexte
> étant refermée, l'usage avance sur **l'appropriation des acteurs** : compléter
> le cycle de vie par la **suppression d'un acteur** du foyer (Create + Read +
> Update livrés ; **Delete** manquant). La suppression est **autorisée** et
> **neutralise les cases orphelines** par repli (la surcharge orpheline cesse de
> primer, la case retombe sur le **fond** — le cycle reprend — ou sur le
> **neutre**), avec **message non bloquant** et **sans réaffectation
> automatique** (cf. règle 6). Elle touche la **config foyer persistée Mongo**
> (palier 5) → **acceptation runtime obligatoire**. Sujet `/2-make-gherkin` =
> `crud-acteurs-suppression`, **puis** `impersonation-bornee` (amorce de
> convenance admin, **pas** l'auth réelle). **Hors make-gherkin, en tête de
> sprint** : le **fix ciblé** du dropdown « Acteur du foyer » (lecture statique à
> repointer sur le store vivant, cf. Risques). Les variantes refus/réaffectation
> ne s'ouvrent qu'à une **porte G1** si le make-gherkin révèle un vrai trou (ex.
> dernier responsable d'un enfant).
...
(dialogs, transfert inclus) » qui l'ont suivi sont **livrés**. L'arbitre d'usage
tient l'ordre : les paliers d'usage d'abord, les paliers **techniques en queue de
...
5. **Config foyer persistante — ajout d'acteurs & survie au redémarrage**
   ...
   **uniquement** ; le reste du domaine (slots / périodes / transferts) **reste en
   ...
   est le sujet du palier 8 « CRUD acteurs — suppression »), **sans** édition du
   cycle de fond ici.*
...
8. **CRUD acteurs — suppression & amorce d'impersonation bornée**
   *(reprise d'usage — **PROCHAIN SUJET**, tiré devant le calendrier navigable)*.
   Compléter le cycle de vie des acteurs par la **suppression** (Create + Read +
   Update livrés ; **Delete** manquant), épic **É2 (Modèle & configuration
   d'acteurs)**. La suppression est **autorisée** et **cadre les cases orphelines
   par neutralisation par repli** : la **surcharge orpheline cesse de primer**, la
   case retombe sur le **fond** (le cycle reprend, cf. priorité surcharge > fond >
   neutre) ou sur le **neutre** si l'index n'est ni mappé ni résolu (sans nom
   fantôme) ; si l'acteur supprimé était **mappé au cycle de fond**, son index
   devient **non mappé → neutre**. Un **message non bloquant** accompagne l'acte ;
   **pas de réaffectation automatique** (règle 6). La config foyer étant
   **persistée Mongo** (palier 5), la suppression touche un **store réel** →
   **acceptation runtime obligatoire**. *Découpe : `crud-acteurs-suppression`
   (rang 1) **puis** une **amorce d'impersonation bornée** (rang 2, épic **É10**)
   — tant que les acteurs ne sont pas des **utilisateurs réels** (auth au palier
   « ouverture de l'accès »), l'utilisateur principal peut **incarner un acteur**
   pour agir en son nom (**convenance admin**, **pas** l'auth complète). Ne pas
   tirer la fondation auth devant l'usage ; l'impersonation reste une amorce.
   **Hors make-gherkin, en tête de sprint** : le fix ciblé du dropdown « Acteur du
   foyer » (cf. Risques).*

9. **Calendrier navigable — navigation, vues prédéfinies & sélection de plage**
   *(reprise produit — **NON LIVRÉ**, séquencé derrière la suppression d'acteur)*.
   Faire du hub `/planning` un **agenda navigable** : se déplacer dans le **passé
   et le futur**, choisir des **vues prédéfinies** (semaine, mois, **4 semaines
   glissantes** par défaut à partir de la semaine en cours), et **sélectionner une
   plage de cases** pour affecter une période sur l'**intervalle** choisi
   (l'affectation par plage rouvre l'écriture en contexte sur plusieurs jours d'un
   coup). *La grille actuelle est une **vue posée non encore navigable** ; ce
   palier l'enrichit sans toucher aux mécaniques d'écriture déjà livrées
   (dialogs). Aucune persistance tirée en avant.*
...
> **Re-séquencement acté.** L'**écriture en contexte par dialogs** (palier 7) est
> désormais **livrée et complète** : la **3ᵉ dialog « Définir un transfert »** et
> le **retrait de la dernière page de saisie dédiée** ont refermé l'épic. Le
> **prochain sujet** est le **CRUD acteurs — tranche suppression** (palier 8),
> **tiré devant** le calendrier navigable par priorité d'usage : la suppression
> d'un acteur porte un observable produit direct, le calendrier reste une
> évolution de la vue. Le palier 8 se découpe en `crud-acteurs-suppression`
> (rang 1) **puis** `impersonation-bornee` (rang 2). Derrière viennent le
> **calendrier navigable** (palier 9, non livré : navigation, vues, sélection de
> plage) puis le **cycle de fond riche** (palier 10, qui rouvre l'ancrage ISO à
> son cadrage). Le **survol enrichi** (palier 11) reste **skippé** faute de
> demande PO, séquencé sans être écarté. La persistance du **reste du domaine**
> reste en queue (palier 17), derrière tout l'usage — seule la persistance de la
> config foyer a été tirée devant (borne).

> **Numérotation — v13 référence unique.** Par rapport à la v12, deux paliers ont
> **permuté** : le **CRUD acteurs — suppression** (jadis palier 9) passe **palier
> 8**, **devant** le **calendrier navigable** (jadis palier 8) qui devient
> **palier 9** ; les paliers suivants sont **décalés d'un cran** en conséquence
> (cycle de fond riche 10, survol enrichi 11, config foyer durable 12, modèle
> d'acteurs 13, immédiat & événements 14, imprévu & échange 15, ouverture de
> l'accès 16, persistance réelle 17, PWA 18). La séquence ci-dessus est la
> **référence unique** et continue ; le swap sera répercuté au `docs/BACKLOG.md`
> à la clôture du sprint.
...
> **Prochain sujet** : **CRUD acteurs — tranche suppression** (`crud-acteurs-suppression`,
> palier 8) — **supprimer un acteur** du foyer, **neutraliser ses cases
> orphelines par repli** (surcharge orpheline → fond → neutre, sans nom fantôme),
> **message non bloquant**, **sans réaffectation automatique** (règle 6) ;
> acceptation **runtime** obligatoire (store Mongo réel). **Puis**
> `impersonation-bornee` (amorce de convenance admin). **Hors make-gherkin, en
> tête de sprint** : fix ciblé du dropdown « Acteur du foyer » (lecture statique →
> store vivant, cf. Risques). C'est ce sujet qui réamorce `/2-make-gherkin` sur
> cette spec.
...
4. **Acteurs « autres » ajoutables, éditables et supprimables** — Un foyer peut avoir N acteurs « autres » (nounou, grands-parents…). Ils sont **ajoutables et éditables** (par les parents ou par l'acteur lui-même), au-delà du seul renommage/recoloriage du seed initial : ajouter un acteur le fait exister dans le foyer et dans la grille (cf. règle 6). Leur **suppression** est le **sujet courant** (palier 8 « CRUD acteurs — suppression », cf. règle 6)

5. **Édition des acteurs (noms + couleurs)** — Les acteurs du foyer (leurs **noms** et leurs **couleurs**) sont **éditables** depuis un écran de configuration, et la **grille (case + légende) relit immédiatement** la configuration éditée : renommer ou recolorier un acteur se voit aussitôt, **partout où l'acteur apparaît** (case, légende, sélecteurs de saisie et mapping du cycle de fond doivent lire le même référentiel vivant). Cette édition s'est livrée d'abord **en mémoire, dans la session** ; sa **durabilité** (survie au redémarrage) est portée par la règle 6 et la règle 30. La survie au redémarrage **est acquise pour la config foyer** : la dette volatile de l'édition s'éteint là, ailleurs elle subsiste (cf. règle 30). *Un point de dette subsiste : un sélecteur de l'écran de config (« Acteur du foyer ») lit une **liste statique** au lieu du **store vivant**, ce qui contredit cette règle après renommage — fix ciblé hors make-gherkin en tête de sprint (cf. Risques).*

6. **Ajout & suppression d'acteur, neutralisation par repli, persistance bornée** — On peut **ajouter** un acteur au foyer (parent / autre / nounou) : l'ajout génère un **identifiant d'acteur stable neuf** (jamais dérivé du libellé d'affichage) et la grille (case + **légende dédoublonnée par identifiant**) le reflète immédiatement. La **configuration du foyer** — référentiel des acteurs : **noms, couleurs, acteurs ajoutés** — est **persistée** derrière les ports de droite par un **adaptateur durable** : elle **survit au redémarrage**. Cette persistance est **bornée à la config foyer** : c'est le **premier client** du store durable, tiré **devant l'usage** parce qu'il porte un observable direct. La **suppression d'un acteur** (Delete) est le **sujet courant** (palier 8) et est **autorisée** — pas de refus « si références existantes » (qui contredirait l'additivité et le repli neutre). Les **cases orphelines** de l'acteur retiré sont **neutralisées par repli** : sa **surcharge cesse de primer** et la case **retombe sur le fond** (le cycle reprend, cf. règles 12 et 15) ou sur le **neutre** si l'index n'est ni mappé ni résolu, **sans nom fantôme** (règles 15/19) ; si l'acteur supprimé était **mappé au cycle de fond**, son index devient **non mappé → neutre** (règles 11/19). La suppression s'accompagne d'un **message non bloquant** (registre avertissement-à-part, règles 16/28). Il n'y a **pas de réaffectation automatique** (ce serait une règle neuve, hors périmètre). La suppression opère sur la config foyer **déjà persistée Mongo** (palier 5) → **acceptation runtime obligatoire** (store réel). *Variantes refus/réaffectation = porte **G1 au make-gherkin uniquement** si un vrai trou émerge (ex. interdire la suppression du dernier responsable d'un enfant). Les transferts dérivés (règle 24) restent invisibles jusqu'au panneau cloche → aucun orphelin de transfert observable séparé ; un transfert ponctuel explicite (règle 25) suit la même neutralisation, à scénariser au make-gherkin (pas un pré-arbitrage de cette spec).* La persistance du **reste du domaine** reste **en queue** (règle 30, borne anti-cliquet)
...
30. **Données derrière les ports, durables — exception bornée pour la config foyer** — Les slots, périodes, transferts, **cycle de fond** et la configuration du foyer vivent derrière des **ports** ; la donnée du foyer n'est jamais une constante figée dans le code. Leur stockage est durable **à terme**, sans toucher au domaine ni aux règles. **Exception bornée actée et livrée** : la **config foyer** (référentiel des acteurs — noms, couleurs, acteurs ajoutés) est **persistée maintenant**, **devant l'usage**, parce qu'elle porte un observable direct (survie au redémarrage) et qu'elle est le **premier client** du store durable. Le **reste du domaine** (slots, périodes, transferts **et le cycle de fond**) reste **en mémoire** (adaptateur InMemory), sa durabilité **séquencée derrière l'usage** : la **borne anti-cliquet** empêche que cette exception entraîne le reste devant l'usage. L'**écriture en contexte** (palier 7, **transfert compris**) l'a confirmée : déplacer la saisie en dialogs n'a tiré **aucune** persistance (slots / périodes / **transferts** restent InMemory). La **suppression d'acteur** (palier 8) opère sur la config foyer **déjà durable** — elle exerce une persistance acquise, sans créer de cliquet. Cette règle porte la **durabilité** ; elle reste **distincte de l'édition** (règle 5) — rendre une donnée éditable (cf. le cycle de fond, éditable mais volatil) n'oblige pas à la rendre durable dans le même incrément, sauf quand, comme pour la config foyer, la durabilité porte un observable direct et reste bornée

## Risques & questions ouvertes

- **L'usage tient la main — ne pas enchaîner un sprint sans valeur produit.** Deux sprints structurels (fondation, hôte d'API) puis sept sprints d'usage (saisie visible, lisibilité & thème, édition des acteurs, config foyer persistante, récurrence des périodes, écriture en contexte slot/période, écriture en contexte transfert) sont derrière nous. Les prochains sujets (CRUD acteurs suppression, impersonation bornée, calendrier navigable, cycle de fond riche, survol enrichi) restent des paliers **d'usage** ; les paliers techniques débloqués (persistance du reste du domaine, PWA, Docker) sont tentants mais **doivent rester derrière l'usage**. La seule persistance tirée devant a été **bornée à la config foyer** (observable direct) : ne pas en faire un cliquet.
- **Prochain sujet — CRUD acteurs, tranche suppression (`crud-acteurs-suppression`, palier 8).** Compléter le cycle de vie des acteurs par la **suppression** (Delete), avec **neutralisation par repli** des cases orphelines (surcharge orpheline → fond → neutre, sans nom fantôme), **message non bloquant** et **sans réaffectation automatique** (règle 6). **Bornes** : suppression **autorisée** (pas de refus « si références ») ; transfert et reste du domaine **restent InMemory** (règle 30), mais la **config foyer est persistée Mongo** (palier 5) → la suppression touche un **store réel** ; **acceptation runtime obligatoire** (front WASM réel + API distante + store Mongo réel, rempart anti vert-qui-ment), **pas** de doublure comme seule preuve. **Puis** `impersonation-bornee` (rang 2, épic É10). **Politique des cases orphelines confrontée à la spec au /2** : si le make-gherkin révèle un **vrai trou** (ex. interdire la suppression du dernier responsable d'un enfant), une variante refus/réaffectation s'ouvre à une **porte G1** — sinon la neutralisation par repli (règle 6) tient sans nouvelle porte.
- **Pilotage au catalogue — retours produit VIDE ce sprint.** Le sprint 12 (transfert en contexte) a été livré et validé au gate G3 **sans aucun retour d'usage déposé** : la priorisation du prochain sujet (suppression d'acteur) dérive du **backlog seul**, sans signal d'usage frais. Point de vigilance : **confirmer le besoin réel de suppression au démarrage du sprint 13** ; ne pas enchaîner un sprint « à vide » sur du catalogue si un retour d'usage plus pressant émerge.
- **Dette dropdown « Acteur du foyer » — fix /3 ciblé hors make-gherkin (tête de sprint 13).** Un écart de cohérence interne à l'écran de configuration (le sélecteur « Acteur du foyer » lit une **liste statique** au lieu du **store vivant des acteurs**) peut faire afficher un libellé **périmé** après renommage. C'est une **dérive vis-à-vis de la règle 5** (« tout lit le même référentiel vivant »), corrigée par un **fix ciblé léger hors make-gherkin** (repointer la lecture sur le store), **embarqué en tête de sprint 13 avant** le sujet suppression ; ce n'est **pas** une règle ni un palier, et **pas** un scénario Gherkin du CRUD (à ne pas confondre avec le sujet). Cible : un acteur renommé/ajouté est aussitôt cohérent **partout** (case, légende, sélecteurs, mapping du cycle).
- **Calendrier navigable non livré (palier 9).** La navigation **passé/futur**, les **vues prédéfinies** (semaine, mois, 4 sem. glissantes) et la **sélection d'une plage de cases** pour affecter une période **restent à livrer** : la grille actuelle est une vue posée, non navigable. Besoin **conservé**, **séquencé derrière la suppression d'acteur** (tirée devant par priorité d'usage) ; à ne pas sous-entendre comme acquis (les mécaniques le marquent « cible »).
...
- **Acceptation runtime obligatoire (rempart anti vert-qui-ment).** Les incréments d'usage sont prouvés sur l'**app réellement câblée** (front WASM + API distante + SignalR + store réel), pas par doublures : le cycle de fond a été accepté en affichant le **fond résolu** sans saisie de période ; l'écriture en contexte (slot, période, **transfert**) a été acceptée en prouvant qu'une saisie **réellement enregistrée** via une dialog réapparaît **positionnée, colorée et nommée** à la **date de la case**, et que la suppression du dernier écran dédié ne laisse **plus aucune route de saisie**. Ce rempart reste la règle pour les prochains incréments : la **suppression d'acteur** doit être prouvée sur le **store Mongo réel** (l'acteur retiré disparaît du store **et** ses cases retombent sur le fond/neutre), pas par doublure ; un test de composant à doublures peut afficher une grille alors que le câblage réel échoue.
- **Borne anti-cliquet à tracer sans déraper.** La persistance devant l'usage est une **exception bornée à la config foyer**. Risque de **cliquet** : que le reste du domaine (slots / périodes / transferts / **cycle de fond**) suive devant l'usage. Garder cette persistance **en queue** (paliers « config foyer durable — reste » puis « persistance réelle ») ; la borne est écrite noir sur blanc (règle 30, et `docs/BACKLOG.md`). L'écriture en contexte (transfert compris) vient de la respecter (aucune persistance tirée) ; la **suppression d'acteur** exerce la persistance **déjà acquise** de la config foyer, sans l'étendre.
