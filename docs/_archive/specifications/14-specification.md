...
> Version 14 · consolide la v13 + docs/sprints/13-crud-acteurs-suppression/99-sprint13-besoins-fin-itération.md. Remplace la v13, qui reste figée en historique.
...
ainsi complet** (Create + Read + Update + Delete). Le **dernier maillon de la
tranche acteurs** — une **impersonation bornée** (incarner un acteur déjà
déclaré, **convenance d'administration**) — est le **prochain sujet d'usage** :
amorce de l'ouverture de l'accès **sans** tirer l'authentification réelle en
avant.
...
> inclus**) » **et « suppression d'acteur »** sont **livrés**. C'est cet arbitre
> qui tire l'**impersonation bornée** (dernier maillon d'usage de la tranche
> acteurs) **devant** le calendrier navigable.
...
> acquise. L'**impersonation bornée** (prochain sujet) ne tire **aucune
> persistance neuve** non plus.
...
> acteurs** s'est découpé en **suppression d'abord (livrée)**, **amorce
> d'impersonation ensuite (prochain sujet)**. Il reste le **garde-fou de secours**
...
> make-gherkin du palier « cycle de fond riche »**. Le **durcissement du gating
> de l'écran de config** (toutes les écritures config gatées, pas seulement le
> bouton supprimer — angle mort Sc.7) est une **quatrième** révision en attente,
> **candidate du make-gherkin de l'impersonation** : la règle 9 n'est **pas**
> révisée d'ici là.

> **Prochain sujet : impersonation bornée (palier 8, tranche 2).** Le CRUD
> acteurs étant complet (suppression livrée), l'usage ferme la **boucle du cycle
> de vie des acteurs** par une **impersonation bornée** : l'admin / parent
> configurateur **incarne un acteur déjà déclaré** du foyer pour voir et agir
> « comme » lui (**convenance d'administration**). **Borne dure (non
> négociable)** : ce n'est **PAS** l'authentification réelle du palier 16 — ni
> OAuth, ni landing, ni comptes, ni sessions, ni prise en main par demande, ni
> droits par rôle après prise en main, qui restent au palier 16 ; l'impersonation
> est un confort d'administration **avant** l'auth, borné à incarner un acteur
> déjà déclaré. **Aucune persistance neuve** n'est tirée en avant (config foyer
> durable acquise au palier 5, reste du domaine InMemory — borne anti-cliquet).
> Le **périmètre exact** (lecture seule vs écriture « au nom de », sortie/retour à
> l'identité réelle, frontière avec l'auth réelle du palier 16) se **cadre au
> make-gherkin**, **non pré-arbitré ici** (candidat **G1** si un vrai trou métier
> apparaît). **Candidat embarquable au même cadrage** : le **durcissement du
> gating de l'écran de config** (cf. Risques, angle mort Sc.7). Sujet
> `/2-make-gherkin` = `impersonation-bornee`. C'est ce sujet qui réamorce
> `/2-make-gherkin` sur cette spec.
...
(dialogs, transfert inclus) » **et « suppression d'acteur »** qui l'ont suivi
sont **livrés**. L'arbitre d'usage tient l'ordre : les paliers d'usage d'abord,
les paliers **techniques en queue de séquence**, derrière tout l'usage — à la
seule **exception bornée** de la persistance de la config foyer, qui a été tirée
devant parce qu'elle portait un observable d'usage direct, et qui est désormais
livrée.
...
8. **CRUD acteurs — suppression (livrée) & impersonation bornée (prochain
   sujet)** *(reprise d'usage — **suppression LIVRÉE**, **impersonation = PROCHAIN
   SUJET**, tirés devant le calendrier navigable)*. Le cycle de vie des acteurs
   est complété par la **suppression** (Create + Read + Update **et Delete**
   désormais livrés), épic **É2 (Modèle & configuration d'acteurs)**. La
   ...
   **acceptation runtime tenue** (l'acteur retiré disparaît du store relu **et**
   après redémarrage ; cases retombées sur fond/neutre ; gating Invité sur le
   bouton supprimer, échec API et propagation SignalR prouvés sur l'app câblée).
   La **2ᵉ tranche — `impersonation-bornee` (épic É10) — est le prochain sujet** :
   ...
   déjà déclaré** pour agir en son nom (**convenance admin**, **pas** l'auth
   complète). Ne pas tirer la fondation auth devant l'usage ; l'impersonation
   reste une **amorce**, **sans persistance neuve**. Périmètre exact (lecture
   seule vs écriture « au nom de », sortie/retour à l'identité réelle) **cadré au
   make-gherkin**, non pré-arbitré ici.

9. **Calendrier navigable — navigation, vues prédéfinies & sélection de plage**
   *(reprise produit — **NON LIVRÉ**, séquencé derrière la tranche acteurs)*.
...
> **Re-séquencement acté.** La **tranche suppression** du palier 8 (`crud-acteurs-suppression`)
> est désormais **livrée** : suppression autorisée, neutralisation par repli des
> cases orphelines, accusé non bloquant, sur **store Mongo réel** (9/9 scénarios
> verts, suite complète 196/196). Le **prochain sujet** est la **2ᵉ tranche du
> palier 8 — `impersonation-bornee`** (épic É10), **tirée devant** le calendrier
> navigable par priorité d'usage : incarner un acteur déjà déclaré ferme la boucle
> du cycle de vie acteurs, alors que le calendrier reste une évolution de la vue.
> Derrière viennent le **calendrier navigable** (palier 9, non livré : navigation,
> vues, sélection de plage) puis le **cycle de fond riche** (palier 10, qui rouvre
> l'ancrage ISO à son cadrage). Le **survol enrichi** (palier 11) reste **skippé**
> faute de demande PO, séquencé sans être écarté. La persistance du **reste du
> domaine** reste en queue (palier 17), derrière tout l'usage — seule la
> persistance de la config foyer a été tirée devant (borne).

> **Numérotation — v14 référence unique.** La numérotation des paliers est
> **inchangée depuis la v13** (le swap palier 8/9 — suppression devant calendrier
> navigable — a déjà été acté et répercuté au `docs/BACKLOG.md`). Le **palier 8**
> voit sa **tranche suppression livrée** et conserve sa **tranche impersonation**
> comme **prochain sujet** ; les paliers suivants (calendrier navigable 9, cycle
> de fond riche 10, survol enrichi 11, config foyer durable 12, modèle d'acteurs
> 13, immédiat & événements 14, imprévu & échange 15, ouverture de l'accès 16,
...
> **Prochain sujet** : **impersonation bornée** (`impersonation-bornee`, palier 8,
> tranche 2, épic É10) — **incarner un acteur déjà déclaré** du foyer (convenance
> d'administration), **sans** OAuth / comptes / sessions / prise en main (auth
> réelle = palier 16) et **sans persistance neuve** (config foyer durable acquise,
> reste du domaine InMemory). Périmètre exact (lecture seule vs écriture « au nom
> de », sortie/retour à l'identité réelle) **cadré au make-gherkin** ; candidat
> embarquable au même cadrage : le **durcissement du gating de l'écran config**
> (angle mort Sc.7, cf. Risques). C'est ce sujet qui réamorce `/2-make-gherkin`
> sur cette spec.
...
8. **Trois types d'acteurs** — Trois types d'accès : Admin (gère tout, y compris la configuration du foyer), Parent (gère les créneaux, lieux, acteurs et invitations de son foyer), Autre (consultation et édition limitée à ses propres informations). L'affichage s'adapte au type. Tant que les acteurs ne sont pas des **utilisateurs réels** (authentification au palier « ouverture de l'accès »), une **impersonation bornée** est le **prochain sujet** (`impersonation-bornee`, épic É10, **2ᵉ tranche du palier 8**, **après** la tranche suppression **livrée**) : l'utilisateur principal peut **incarner un acteur déjà déclaré** pour agir en son nom — **convenance admin**, **pas** l'authentification complète, qui n'est jamais tirée devant l'usage

9. **Modification réservée aux parents et à l'admin** — Seul un Parent (ou l'Admin) peut créer, éditer ou supprimer un créneau, une période **ou le cycle de fond** ; un acteur « Autre » n'édite que ses propres informations. Depuis que l'écriture se fait **en contexte** (palier 7), le **point d'application** de ce droit est le **déclencheur unique** — le **menu ouvert au clic sur une case** — rendu **conditionnel sur le rôle** : un Invité (consultation seule) ne voit pas le menu et n'ouvre aucune dialog, un Parent l'ouvre. Ce gating réutilise le **contexte rôle existant** et reste **mutualisé** sur le déclencheur quelle que soit l'entrée (slot, période, **transfert**) ; il ne tire **ni authentification ni impersonation** (séquencées à leurs paliers). *L'écran de configuration n'applique aujourd'hui ce gating qu'au **bouton supprimer** (posé au palier 8, `@if EstParent`) ; l'**ajout / l'édition d'acteur / l'édition du cycle de fond** y restent ouverts à un Invité — angle mort signalé (Sc.7). Le **durcissement complet du gating config** est une révision **séquencée**, candidate du make-gherkin de l'impersonation (cf. Risques) ; cette règle n'est **pas** révisée d'ici là.*
...
30. **Données derrière les ports, durables — exception bornée pour la config foyer** — Les slots, périodes, transferts, **cycle de fond** et la configuration du foyer vivent derrière des **ports** ; la donnée du foyer n'est jamais une constante figée dans le code. Leur stockage est durable **à terme**, sans toucher au domaine ni aux règles. **Exception bornée actée et livrée** : la **config foyer** (référentiel des acteurs — noms, couleurs, acteurs ajoutés) est **persistée maintenant**, **devant l'usage**, parce qu'elle porte un observable direct (survie au redémarrage) et qu'elle est le **premier client** du store durable. Le **reste du domaine** (slots, périodes, transferts **et le cycle de fond**) reste **en mémoire** (adaptateur InMemory), sa durabilité **séquencée derrière l'usage** : la **borne anti-cliquet** empêche que cette exception entraîne le reste devant l'usage. L'**écriture en contexte** (palier 7, **transfert compris**) l'a confirmée : déplacer la saisie en dialogs n'a tiré **aucune** persistance (slots / périodes / **transferts** restent InMemory). La **suppression d'acteur** (palier 8) a opéré sur la config foyer **déjà durable** — elle a **exercé** une persistance acquise, **sans créer de cliquet** ; l'**impersonation bornée** (prochain sujet) ne tire **aucune persistance neuve**. Cette règle porte la **durabilité** ; elle reste **distincte de l'édition** (règle 5) — rendre une donnée éditable (cf. le cycle de fond, éditable mais volatil) n'oblige pas à la rendre durable dans le même incrément, sauf quand, comme pour la config foyer, la durabilité porte un observable direct et reste bornée

## Risques & questions ouvertes

- **L'usage tient la main — ne pas enchaîner un sprint sans valeur produit.** Deux sprints structurels (fondation, hôte d'API) puis huit sprints d'usage (saisie visible, lisibilité & thème, édition des acteurs, config foyer persistante, récurrence des périodes, écriture en contexte slot/période, écriture en contexte transfert, **suppression d'acteur**) sont derrière nous. Les prochains sujets (impersonation bornée, calendrier navigable, cycle de fond riche, survol enrichi) restent des paliers **d'usage** ; les paliers techniques débloqués (persistance du reste du domaine, PWA, Docker) sont tentants mais **doivent rester derrière l'usage**. La seule persistance tirée devant a été **bornée à la config foyer** (observable direct) : ne pas en faire un cliquet.
- **Prochain sujet — impersonation bornée (`impersonation-bornee`, palier 8, tranche 2, épic É10).** Fermer la boucle du cycle de vie des acteurs : l'admin / parent configurateur **incarne un acteur déjà déclaré** du foyer (convenance d'administration). **Borne dure (non négociable)** : **PAS** l'authentification réelle du palier 16 (ni OAuth, ni landing, ni comptes, ni sessions, ni prise en main par demande, ni droits par rôle après prise en main) ; **aucune persistance neuve** tirée en avant (config foyer durable acquise, reste du domaine InMemory). À cadrer **au make-gherkin** (non pré-arbitré ici) : périmètre exact (lecture seule vs écriture « au nom de »), sortie/retour à l'identité réelle, ce que l'impersonation **n'ouvre pas** (frontière avec l'auth réelle). **Candidat embarquable au même cadrage** : le durcissement du gating config (puce ci-dessous). Risque de **scope creep** vers l'auth du palier 16 (candidat **G1** si un vrai trou métier apparaît) ; **couplage É2 ↔ É10** à expliciter (la création d'acteur avec email amorce le compte — ce qui est dans la tranche vs reporté au palier 16).
- **Durcissement du gating de l'écran config (angle mort Sc.7) — révision de règle 9 séquencée.** Le palier 8 a posé le gating Invité (`@if EstParent`) **uniquement** sur le **bouton supprimer** ; l'**ajout / l'édition d'acteur / l'édition du cycle de fond** de l'écran `ConfigurationFoyer` **restent ouverts à un Invité**. Décider si **toutes** les écritures config doivent être gatées (cohérence règle 9) est une **révision de règle hors boucle**, **candidate du make-gherkin de l'impersonation** (même écran, même notion de rôle/identité). **Décision CP/métier** (pas un retour produit PO) : la règle 9 **n'est pas révisée d'ici là** ; inscrire le gating complet en règle satisfaite sans scénario ni acceptation runtime serait un pré-arbitrage interdit. À confirmer au cadrage.
- **Calendrier navigable non livré (palier 9).** La navigation **passé/futur**, les **vues prédéfinies** (semaine, mois, 4 sem. glissantes) et la **sélection d'une plage de cases** pour affecter une période **restent à livrer** : la grille actuelle est une vue posée, non navigable. Besoin **conservé**, **séquencé derrière la tranche acteurs** (suppression livrée, impersonation prochaine) ; à ne pas sous-entendre comme acquis (les mécaniques le marquent « cible »).
- **Stabilisation des flakes temps-réel SignalR (P2 — action de méthode).** Au palier 8, la touche du composant config partagé (`ConfigurationFoyer` : boutons + gating) a exposé une **course latente `UnknownEventHandlerId`** dans **7 tests `*TempsReel*` préexistants** interagissant avec un `select` sans garde d'énumération → un **garde déterministe `WaitForState`** a été posé sur ces 7 tests (standard des tests frères). Le **rétrofit de tous les tests config/grille** reste à **généraliser** : ils **vivent dans les tests, pas dans `src/`** (fiabilité de test, pas un bug produit ni une règle), à porter en **retro-sprint**. C'est un **prérequis de fait** de l'édition concurrente (ci-dessous) : driver celle-ci sur une fondation instable produirait des scénarios **flaky par construction**.
...
- **Pilotage au catalogue — retours produit VIDE au sprint 13.** Le sprint 13 (suppression d'acteur) a été livré et validé au gate G3 **sans aucun retour d'usage déposé** (chemin nominal, goal 9/9 atteint, aucune anomalie) : la priorisation du prochain sujet (impersonation bornée) dérive du **backlog seul**, sans signal d'usage frais. Point de vigilance : **confirmer le besoin réel d'impersonation au démarrage du sprint** ; ne pas enchaîner un sprint « à vide » sur du catalogue si un retour d'usage plus pressant émerge.
...
- **Diffusion déclenchée par l'écriture** — Garantir qu'une écriture aboutie déclenche bien l'actualisation temps réel des autres écrans, sans jamais écrire par le canal de diffusion ; validé en usage réel pour le cycle de fond (deux écrans convergent sans rechargement), **acquis pour les dialogs en contexte** (l'ouverture d'une dialog n'interfère pas avec le rafraîchissement de fond) **et pour la suppression d'acteur** (un second écran voit la case orpheline retomber sur le fond et la légende se dédoublonner, sans rechargement). La stabilité sous exécution parallèle des tests temps-réel reste à durcir (P2).
