...
| 2 | [Renommer un acteur déjà semé met à jour la grille](02-renommer-acteur-met-a-jour-grille.md) | `@nominal` 🖥️ IHM · caractérisation (s08) + runtime `ihm-builder` | ✅ GREEN (caractérisation) runtime `ihm-builder` (les 5 cases du 1er→5 juin + légende suivent le renommage sans rechargement, app réellement câblée) | 1/1 runtime | ✅ GREEN (caractérisation) |
| 3 | [L'ajout et l'édition survivent au redémarrage](03-ajout-edition-survivent-redemarrage.md) | `@nominal` 🖥️ pivot durabilité · **intégration Mongo réel (Docker)** + runtime `ihm-builder` | ✅ GREEN (**intégration sur Mongo réel** : seed-once amorçage + cardinal anti-re-seed + survie ajout/édition après redémarrage, case comme légende ; skip propre si Docker/Mongo absent) | 3/3 | ✅ GREEN |
| 4 | [Un acteur ajouté apparaît en légende une fois une période affectée](04-acteur-ajoute-apparait-en-legende.md) | `@nominal` · caractérisation `tdd-auto` + runtime `ihm-builder` | ✅ GREEN (caractérisation backend : l'id neuf circule jusqu'à légende/cases) · ⏳ runtime `ihm-builder` (entrée légende « Carla » rose sur id neuf) | 1/1 | ✅ GREEN (caractérisation) |
| 5 | [Un acteur ajouté sans couleur retombe sur la teinte neutre](05-acteur-sans-couleur-teinte-neutre.md) | `@limite` · caractérisation `tdd-auto` (contrat palette) + runtime `ihm-builder` | ✅ GREEN (caractérisation backend : sans couleur → teinte neutre grise, nom conservé) · ⏳ runtime `ihm-builder` (case + légende « Papy Jo » gris) | 1/1 | ✅ GREEN (caractérisation) |
| 6 | [Un acteur ajouté sans période ne crée pas d'entrée fantôme](06-acteur-sans-periode-pas-de-fantome.md) | `@limite` · caractérisation `tdd-auto` (s07/s08) + runtime `ihm-builder` | ✅ GREEN (caractérisation backend : énumération présente, légende/case vides pour Carla sans période) · ⏳ runtime `ihm-builder` (présent liste config, absent légende/case) | 1/1 | ✅ GREEN (caractérisation) |
| 7 | [Deux acteurs de même libellé reçoivent deux identifiants distincts](07-deux-libelles-identiques-ids-distincts.md) | `@limite` · caractérisation `tdd-auto` (id opaque + dédoublonnage s07) | ✅ GREEN (caractérisation backend : deux ajouts « Carla » → deux ids opaques distincts, énumération à deux entrées, légende dédoublonnée par id sans fusion sur le libellé) | 1/1 | ✅ GREEN (caractérisation) |
| 8 | [Ajouter un acteur sans nom est refusé](08-ajouter-sans-nom-refuse.md) | `@erreur` 🖥️ IHM · backend `tdd-auto` + runtime `ihm-builder` | ✅ GREEN backend (garde nom vide / tout-espaces : refus motif clair, aucun id, liste inchangée + caractérisation absence d'effet de bord) · ✅ GREEN runtime `ihm-builder` (message « le nom ne peut pas être vide » surfacé à l'écran de config, liste inchangée, app réellement câblée) | 3/3 + runtime | ✅ GREEN (backend + runtime) |
| 9 | [Ajout impossible si le service de configuration est injoignable](09-service-injoignable-ajout-impossible.md) | `@erreur` 🖥️ IHM · driver runtime `ihm-builder` (backend néant) | ✅ GREEN runtime `ihm-builder` (échec clair « service injoignable », saisie Carla/rose conservée, aucun acteur enregistré ; canal d'ajout `catch (HttpRequestException)`) · driver transport déterministe anti-flake Docker (test d'édition s08 rendu robuste, prouvé stable ≥5× Docker actif) | runtime | ✅ GREEN (runtime) |
