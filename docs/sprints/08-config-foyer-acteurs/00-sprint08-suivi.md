...
| 7 | [Deux écrans renomment : dernière écriture gagne, grilles convergent](07-deux-ecrans-derniere-ecriture-gagne.md) | `@limite` 🖥️ IHM · caract. `tdd-auto` + driver runtime `ihm-builder` | ⏳ Pending | 1/1 | 🔶 backend caract. (1/1) — runtime IHM ⏳ `ihm-builder` |
| 8 | [Renommer avec un nom vide : édition refusée, ancien nom conservé](08-nom-vide-edition-refusee.md) | `@erreur` 🖥️ IHM · backend `tdd-auto` + runtime `ihm-builder` | ⏳ Pending | 3/3 | 🔶 backend ✅ (3/3 : 2 drivers garde + 1 caract.) — runtime IHM ⏳ `ihm-builder` |
...
**Acceptation runtime IHM** : **6/8** (Sc.1 ✅ — `FrontWasmConfigRenommerActeurTempsReelTests` ;
...
`FrontWasmConfigHorsFenetrePasDeFantomeTempsReelTests`).
