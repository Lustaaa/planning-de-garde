...
>
> **Gate G3 (4e passage) ÉCHOUÉ** (navigateur 5292, foyer réel Mia/Charlie) : « sur le planning de
> Charlie, Cyril/Esther/Mélanie apparaissent alors que seule Mélanie correspond à Charlie ».
> **Diagnostic runtime** (store Mongo réel) : la collection `cycle_de_fond` ne contenait que des cycles
> **legacy partagés** (`EnfantId=""` + un doc `undefined` pré-champ) ; **aucun** cycle scopé à Mia/Charlie
> → `CycleCourant(charlie)` **retombait sur le bucket `""`** (fallback de LECTURE laissé en place). **Cause
> = A** (fuite de résolution), aggravée par **B** (données legacy `""`). **Correctif** : retrait du repli
> `""` dans `CycleCourant` des 3 providers (InMemory/Mongo/Fake) — un enfant NON-NULL ne lit QUE son cycle,
> sans cycle propre → **NEUTRE** (repli s13) ; le chemin legacy `enfantId=null` (lit `""`) préservé pour les
> tests mono-enfant explicites. **Sc.17 @back** (RED : Tom sans cycle propre affichait le cycle `""`). Données
> legacy `""`/`undefined` rendues **inertes** (jamais lues pour un enfant précis ; l'app passe toujours un enfant).
...
| 17 | Enfant SANS cycle propre résout NEUTRE (jamais le cycle d'un autre / le cycle legacy '') | @back | ✅ |
...
### Correctif gate G3 (4e passage) — la LECTURE du cycle ne retombe plus sur le bucket partagé ''

```gherkin
@back @vert
Scénario 17 — Un enfant SANS cycle propre résout NEUTRE (jamais le cycle d'un autre / le cycle '')
  Étant donné un cycle propre à "Léa" (Alice) ET un cycle legacy PARTAGÉ "" (Bob), "Tom" sans cycle propre
  Quand je résous la grille pour "Tom"
  Alors "Tom" résout NEUTRE (personne assignée) — jamais Bob (cycle partagé ""), jamais Alice (cycle de Léa)
  Et "Léa" continue de résoudre SON cycle (Alice)
  Et l'isolation est prouvée sur InMemory (et implicitement Mongo : même retrait du repli "")
  # diagnostic runtime : « Charlie » (sans cycle propre) affichait le cycle legacy "" — repli de LECTURE retiré
```
