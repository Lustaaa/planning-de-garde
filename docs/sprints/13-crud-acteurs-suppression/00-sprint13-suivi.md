...
| 2 | [Surcharge orpheline : la case retombe sur le fond](02-surcharge-orpheline-retombe-fond.md) | `@limite` `@driver` · backend `tdd-auto` | ⏳ Pending (frontière Application : après suppression, la case retombe sur le responsable de fond) | 0/1 | ⏳ Pending |
| 3 | [Surcharge orpheline sur index non résolu : repli neutre sans nom fantôme](03-surcharge-orpheline-retombe-neutre.md) | `@limite` `@caractérisation` · backend `tdd-auto` (⚠️ early green) | ⏳ Pending (frontière Application : repli neutre, aucun nom — couvert par Sc.2 + contrat index non mappé) | 0/1 | ⏳ Pending |
| 4 | [Acteur mappé au cycle de fond : index non mappé → neutre](04-acteur-mappe-fond-index-non-mappe.md) | `@limite` `@driver` · backend `tdd-auto` | ⏳ Pending (frontière Application : fond orphelin → index non mappé → neutre, sans nom fantôme) | 0/1 | ⏳ Pending |
| 5 | [Supprimer un acteur absent ou déjà supprimé : no-op qui réussit](05-suppression-idempotente.md) | `@erreur` `@caractérisation` · backend `tdd-auto` (⚠️ early green) | ⏳ Pending (frontière Application : DELETE idempotent — succès sans effet, aucune erreur) | 0/1 | ⏳ Pending |
| 6 | [Depuis l'écran de config : bouton supprimer → liste, légende, accusé](06-ihm-bouton-supprimer-liste-legende.md) | `@nominal` 🖥️ IHM `@caractérisation` · runtime `ihm-builder` | ⏳ Pending (runtime : front WASM + API distante + Mongo réel — Nounou quitte la liste relue + légende dédoublonnée + accusé « Acteur supprimé » + absent du store Mongo) | 0/0 | ⏳ Pending |
