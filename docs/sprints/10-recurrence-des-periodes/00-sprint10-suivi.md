...
| 6 | [Deux parents éditent le cycle en même temps : dernière écriture gagne](06-deux-parents-editent-derniere-ecriture-gagne.md) | `@limite` 🖥️ IHM · caractérisation `tdd-auto` + runtime driver `ihm-builder` | ✅ GREEN backend (caractérisation — early green attendu : deux `DefinirCycleHandler` partageant un `CycleDeFondEnMemoire` singleton règlent l'index pair l'un après l'autre, les deux réussissent sans rejet, la dernière écrase → ISO 28 résout Parent C vert en case et en légende ; absence de version/jeton optimiste documentée, patron s08 Sc.7) · ⏳ runtime `ihm-builder` (convergence des deux grilles vers Parent C vert **sans rechargement**, SignalR) | 1/1 | ✅ GREEN (backend) |
| 7 | [Définir un cycle de zéro semaine est refusé](07-cycle-zero-semaine-refuse.md) | `@erreur` 🖥️ IHM · backend `tdd-auto` (1 driver + 1 caract.) + runtime `ihm-builder` | ✅ GREEN backend (garde N ≥ 1 conditionnelle dans `DefinirCycleHandler` : `DefinirCycleCommand(0,…)` → `Result.Echec("le cycle doit compter au moins une semaine")` avant toute écriture/diffusion ; cycle N=2 précédent intact, grille résout encore l'alternance A/B ; nominal N≥1 toujours accepté) · ⏳ runtime `ihm-builder` (message « le cycle doit compter au moins une semaine » à l'écran) | 2/2 | ✅ GREEN (backend) |
