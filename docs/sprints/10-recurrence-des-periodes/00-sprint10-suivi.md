...
**IHM finale (construction, `ihm-builder`)** — l'IHM restante non couverte par un scénario IHM mené
RED→GREEN est en place, **sans aucune règle métier dans l'UI** (l'écran appelle les use cases via le
canal HTTP, la grille rend le read model) :
- **Écran de configuration du foyer** (`ConfigurationFoyer.razor`) — section « Cycle de fond » :
  saisie du nombre de semaines N (`champ-nombre-semaines`, `min=1`) + un sélecteur de responsable par
  index ISO (`champ-cycle-index-{i}`). Le mapping index→responsable est désormais **alimenté par les
  acteurs PERSISTÉS du foyer** (énumérés depuis le store via `api/foyer/acteurs`, comme la liste des
  acteurs en bas d'écran), **plus** la liste statique front : un acteur renommé / fraîchement ajouté
  est aussitôt sélectionnable comme responsable de fond, **sur son identifiant stable** (règle 19).
  Formulaire posé en Sc.8 réutilisé (POST `/api/canal/definir-cycle`, gestion refus métier Sc.7 +
  service injoignable Sc.8). Diffusion SignalR réelle réutilisée (jamais reconstruite).
- **Grille `PlanningPartage.razor`** — rend les caractérisations Sc.2/4/5 comme **observables de rendu**
  du read model `GrilleAgendaQuery` déjà câblé et prouvé au runtime (Sc.1/3/6) : surcharge prime puis le
  fond reprend (Sc.2), index sans responsable → nom masqué + teinte neutre (Sc.4), N=1 → même
  responsable partout (Sc.5). **Aucun écran neuf** : ces observables passent par la grille existante,
  la priorité de résolution est portée par la projection, pas par l'UI.
- **Ergonomie / thème s07** : pastilles de couleur cohérentes (case, légende, liste d'acteurs),
  troncature + survol du nom long, légende dédoublonnée masquée si vide — inchangés, étendus au fond.
