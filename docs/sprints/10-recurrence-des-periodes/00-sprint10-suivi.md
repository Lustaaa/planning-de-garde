...
| 2 | [Une surcharge ponctuelle prime sur le fond puis le cycle reprend](02-surcharge-prime-puis-cycle-reprend.md) | `@nominal` 🖥️ IHM · caractérisation `tdd-auto` (non-régression) + runtime `ihm-builder` | ⏳ backend (⚠️ early green — priorité structurelle `periode is null` first : surcharge 8/07 = Parent B, fond 7 & 9/07 = Parent A) · ⏳ runtime `ihm-builder` | 0/1 | ⏳ Pending |
| 3 | [Inverser le mapping du cycle met à jour la grille sans rechargement](03-inverser-mapping-met-a-jour-grille.md) | `@nominal` 🖥️ IHM · caractérisation `tdd-auto` + runtime driver `ihm-builder` | ⏳ backend (⚠️ early green — re-projection après ré-définition du mapping) · ⏳ runtime `ihm-builder` (grille suit **sans rechargement**, SignalR) | 0/1 | ⏳ Pending |
| 4 | [Un index de cycle sans responsable retombe sur la teinte neutre](04-index-sans-responsable-teinte-neutre.md) | `@limite` · caractérisation `tdd-auto` (contrat de repli) + runtime `ihm-builder` | ⏳ backend (⚠️ early green — index non mappé → neutre sans nom ni légende, comme `CouleurDe` ; driver si indexation en dur) · ⏳ runtime `ihm-builder` | 0/1 | ⏳ Pending |
