...
|---|---|---|---|---|
| 1 | Should_confirmer_la_pose_du_slot_et_exposer_ses_bornes_de_22h_a_7h_le_lendemain_When_le_slot_franchit_minuit | conditional (4) | Contredit une garde de durée naïve fondée sur l'heure seule : un slot qui franchit minuit (fin calendaire > début) reste de durée positive et doit réussir | ⚠️ EARLY GREEN |
...
- Test #1 (⚠️ EARLY GREEN) : la garde du Sc.2 compare déjà l'instant calendaire complet (`fin <= début`), implémentation la plus simple choisie alors. Aucun rouge piloté, aucun changement de code de prod nécessaire ; le test est conservé comme caractérisation `@limite` verrouillant la distinction d'avec le `@erreur`.
