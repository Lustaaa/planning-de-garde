...
> Version 06 · consolide la v05 + docs/sprints/05-host-api-separable/99-sprint05-besoins-fin-itération.md. Remplace la v05, qui reste figée en historique.
...
> est **permanent** et **a repris la main** : la fenêtre d'investissement de
> fondation est refermée (cf. ci-dessous).
...
> détaché, front WASM autonome). L'arbitre d'usage **reprend la main** dès le
...
au titre de l'exception bornée ; il est **refermé**. L'arbitre d'usage tient
désormais l'ordre : les paliers d'usage d'abord, les paliers **techniques en
queue de séquence**, derrière tout l'usage.
...
2. **Saisie visible — la saisie réapparaît à la bonne date et en couleur du
   parent** *(reprise d'usage, prochain sujet)* — une saisie posée **réapparaît
   ...
   l'acteur, pas sur son libellé d'affichage). *Deux choses distinctes, à ne pas
   confondre : (A) la **date par défaut = aujourd'hui** est une **évolution
   d'ergonomie de saisie** — les formulaires portent encore une date figée, la
   saisie tombe hors de la fenêtre affichée ; aucun comportement vert n'est cassé.
   (B) la **couleur du parent** repose sur une règle déjà actée (résolution sur
   identifiant stable, repli neutre sinon) ; le **défaut « gris »** est une
   **non-conformité d'implémentation** (un libellé fourni à la place de
   l'identifiant fait retomber la case sur le neutre), à **confronter au code
   courant et localiser** avant réparation. Lève d'un coup les deux défauts liés
   du faux bug « les saisies n'apparaissent pas ».* **Prochain sujet pour
   `/2-make-gherkin`.**
...
> **Prochain sujet** : palier 2, **saisie visible** — date par défaut = aujourd'hui
> et couleur du parent (identité acteur ↔ palette).
...
20. **Transfert dérivé par défaut** — Le transfert d'un enfant (qui dépose, qui récupère, à quelle heure) est dérivé automatiquement du planning par défaut : c'est le cas nominal, sans saisie

21. **Transfert modifiable et ponctuel** — Un transfert auto-calculé peut être modifié, et des transferts ponctuels peuvent être proposés ou ajoutés à l'exception (urgence, changement d'emploi du temps)

### Modifications & notifications

22. **Modification directe** — Un Parent applique son changement immédiatement ; les autres acteurs concernés sont notifiés (pas de workflow de validation). (Une demande de workflow demande/accord avant réaffectation d'une période à l'autre parent est en attente comme révision de règle, rattachée au palier « imprévu & échange » — cf. Risques & questions ouvertes.)

23. **Notifications & événements à venir** — Les notifications in-app couvrent les changements de planning et les rappels de transfert ; les transferts et changements à venir sont consultables dans un panneau d'événements accessible via une cloche

### Accès aux données & exploitation

24. **Écriture par le canal, échec clair si l'API est injoignable** — Toute écriture passe par le **canal requête/réponse** vers l'API distante ; aucune vue n'écrit le domaine en direct. Si l'API est **injoignable**, la commande échoue **clairement** (message à l'écran, saisie **non appliquée** et conservée à resoumettre), **sans mise en file ni rejeu** à ce stade. Le hors-ligne rejouable (cache + file d'écritures) est un palier technique ultérieur, derrière l'usage

25. **API explorable et origine du front autorisée** — L'API expose un **document de description** (OpenAPI) **et** une **UI interactive** d'exploration des endpoints (type Swagger-UI / Scalar), et autorise l'**origine du front** (CORS). C'est un garde-fou d'outillage, sans observable métier : aucune règle de gestion n'en dépend
...
- **L'usage reprend la main — ne pas enchaîner un 3ᵉ sprint sans valeur produit.** Deux sprints structurels d'affilée (fondation, hôte d'API) sont derrière nous. Le prochain sujet (« saisie visible ») est le **premier d'usage réel** ; les paliers techniques débloqués (persistance, PWA, Docker) sont tentants mais **doivent rester derrière l'usage**. Les laisser passer devant ferait un 3ᵉ sprint sans valeur observable, à l'encontre de l'arbitre.
- **Faux bug « saisies invisibles » vs vrai défaut « gris ».** Le palier « saisie visible » mêle deux choses à **séparer** au make-gherkin : (A) la **date par défaut = aujourd'hui** est une **évolution d'ergonomie** — la saisie n'est pas perdue, elle tombe hors de la fenêtre faute de date par défaut (les formulaires portent encore une date figée) ; aucun comportement vert n'est cassé. (B) la **couleur du parent** est un **défaut d'implémentation confirmé** : un libellé fourni à la place de l'identifiant stable fait retomber la case sur le neutre (gris). Seul (B) est à **confronter au code courant et localiser** (`fichier:lignes`) avant réparation ; la résolution sur identifiant stable (règle 17) reste valide.
