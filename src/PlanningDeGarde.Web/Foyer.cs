...
public static class Foyer
{
    ...
    /// non le libellé qui retombait sur le gris neutre. Les libellés portent les
    /// noms RÉELS du foyer (Alice/Bruno) — plus aucun « Parent A / Parent B » fictif exposé
    /// ; l'unification sur le store vivant des acteurs déclarés est portée par l'IHM.
    /// </summary>
...
/// énumération qui fait apparaître un acteur fraîchement ajouté, nom et pastille de couleur.
/// Le <see cref="Type"/> (Admin / Parent / Autre) est surfacé en lecture seule depuis le seed
/// ; défaut <see cref="TypeActeur.Parent"/> si absent (acteur ajouté en session). Le
/// <see cref="RoleId"/> est l'identifiant stable du rôle du référentiel porté par l'acteur, ou
...
/// lecture : identifiant stable opaque (clé, jamais le libellé) + libellé d'affichage éditable +
/// flag <see cref="EstRoleParent"/> « est un rôle parent » (source de vérité de l'éligibilité
...
/// (GET /api/foyer/cycles) : index de semaine (0.N-1) → identifiant stable du responsable de
/// fond. Alimente le TABLEAU en lecture seule de l'onglet Cycle, qui rend visibles toutes les
/// affectations déclarées (y compris celles auparavant invisibles) ; le nom est
...
public sealed record ActiviteFoyer(string Id, string Libelle, string Adresse = "")
{
    ...
    /// résolus en prénoms par la colonne « Enfants liés ». Propriété <c>init</c> peuplée depuis le JSON
...
/// (GET /api/foyer/enfants) : identifiant stable opaque (clé, bindé par les sélecteurs, jamais le
...
/// remplace le fantôme <c>Session.EnfantId</c> transmis à l'aveugle.</summary>
public sealed record EnfantFoyer(string Id, string Prenom)
{
    /// <summary>Parents-acteurs liés (0.2), chacun avec son <b>rôle-du-lien</b> (père / mère /
    /// parent-libre) — résolus en noms + rôle par la colonne « Parents liés ». Propriété <c>init</c>
...
/// : identifiant stable opaque (clé, jamais l'email) + email + statut (« inactif » / « actif ») +
...
/// <summary>Un parent lié restitué dans le <b>graphe foyer</b> (GET /api/foyer/graphe) : identifiant
/// stable du parent-acteur + son <b>nom d'affichage</b> déjà résolu côté API (jamais un libellé en dur) +
/// son <b>rôle-du-lien</b> (père / mère / parent-libre). Branche « nom (rôle-du-lien) » sous l'enfant
...
/// <summary>Un enfant restitué comme <b>racine</b> du graphe foyer (GET /api/foyer/graphe) : identifiant
...
/// 0 parent accepté). Propriété <c>init</c> peuplée depuis le JSON quand présente, sinon liste vide.</summary>
public sealed record GrapheEnfant(string EnfantId, string Prenom)
{
    ...
    /// <summary>Statut de complétude du couple déjà calculé côté API (GET /api/foyer/graphe) et
    /// peuplé depuis le JSON. Alimente le badge de complétude en LECTURE SEULE. Défaut
