...
/// <summary>Vue d'un rôle du référentiel (id + libellé + flag « est rôle parent »).</summary>
public sealed record RoleFoyerVue(string Id, string Libelle, bool EstRoleParent);

/// <summary>Vue d'une activité du référentiel (id + libellé + adresse + enfants liés).</summary>
public sealed record ActiviteFoyerVue(string Id, string Libelle, string Adresse, IReadOnlyCollection<string> EnfantsLies);

/// <summary>Vue d'un enfant du référentiel (id + prénom + parents liés avec rôle-du-lien).</summary>
public sealed record EnfantFoyerVue(string Id, string Prenom, IReadOnlyCollection<ParentLie> ParentsLies);

/// <summary>Vue d'un compte utilisateur du foyer (id + email + statut + acteur associé).</summary>
...
/// <summary>Vue d'un cycle de fond DÉCLARÉ (onglet Cycle de la config).</summary>
public sealed record CycleFoyerVue(int IndexSemaine, string ResponsableId);

/// <summary>Vue d'une notification de la CLOCHE : événement du journal OU proposition d'échange actionnable.</summary>
