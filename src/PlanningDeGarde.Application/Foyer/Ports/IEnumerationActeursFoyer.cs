...
/// un acteur fraîchement ajouté. Les noms et couleurs se résolvent ensuite sur
...
public interface IEnumerationActeursFoyer
{
    ...
    /// seule</b> depuis la déclaration seed du foyer. Un acteur absent de la
    ...
    /// anti-cliquet). Sert à piloter le rôle de l'identité effective lors d'une impersonation
    ...
    /// et à l'affichage du rôle courant d'un acteur.
    /// </summary>
    string? RoleDe(string acteurId);

    /// <summary>
    /// Adresse de résidence de l'acteur (résolue sur son identifiant stable), ou <c>null</c> s'il n'en
    /// porte aucune (attribut <b>optionnel</b>). Champ de modèle neuf persisté au même titre que le
