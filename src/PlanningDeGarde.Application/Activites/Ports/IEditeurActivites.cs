...
/// sera ajoutée au scénario suivant (borne YAGNI : ne fait qu'<see cref="Ajouter"/>).
/// </summary>
public interface IEditeurActivites
{
    ...
    /// son lieu (borne).</summary>
    void Supprimer(string lieuId);

    /// <summary>Affecte un nouveau libellé à l'activité identifiée de façon stable (surface distincte
    /// de l'adresse — renommer ne touche jamais l'adresse). L'identifiant stable reste la clé.</summary>
    void Renommer(string activiteId, string libelle);

    /// <summary>Affecte une <b>adresse</b> à l'activité identifiée de façon stable (miroir strict
    /// de l'adresse acteur) : surface <b>optionnelle</b> distincte du libellé — la changer ne touche
    ...
    /// <summary>Lie un <b>enfant</b> (référentiel) à l'activité identifiée de façon stable (lien N-M,
    /// miroir du lien enfant↔parent) : enrichissement (l'id de l'activité reste la clé). Ajoute
    ...
    /// <summary>Retire le lien d'un <b>enfant</b> vers l'activité identifiée de façon stable :
