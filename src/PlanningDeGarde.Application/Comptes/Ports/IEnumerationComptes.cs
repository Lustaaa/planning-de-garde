...
/// config foyer, miroir du référentiel de rôles) : énumère les comptes déclarés — chacun porté
...
/// défaut (l'activation viendra avec la prise en main de compte).</summary>
...
/// du mot de passe (facteur local, — jamais le clair ; <c>null</c> pour un compte sans
/// mot de passe, ex. email-only / OAuth). L'acteur est <c>null</c> quand le compte est
/// <b>désassocié</b> (repli après suppression de l'acteur associé) — jamais un compte fantôme
...
public sealed record CompteUtilisateur(string Id, string Email, StatutCompte Statut, string? ActeurId, string? MotDePasseHache = null)
{
    ...
    /// portée par l'agrégat, Tell-Don't-Ask — sens OFF). Seul le statut change ; email et acteur
