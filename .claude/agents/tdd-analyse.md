...
`docs/sprints/<sujet>/` — un `00-suivi.md` (tableau de bord) + un fichier `NN-slug.md`
...
- **`00-suivi.md`** — tableau de bord global : le **Cadrage scaffolding** (en blockquote)
...
- **un `NN-slug.md` par scénario Gherkin** (numéro à 2 chiffres + slug kebab-case du
  ...
  `00-suivi.md`, la ligne **Acceptation (BDD)** (test FLFI de la boucle externe), la
...
- **Ne PAS créer/modifier d'autre fichier** que `00-suivi.md` + les `NN-slug.md` du
  répertoire de suivi. En particulier, **ne JAMAIS toucher** un `NN-retours.md`
  (retours utilisateur manuels) ni un `99-besoins-fin-itération.md` (backlog de l'étage
  `/4-retours`) présents dans le dossier — ce sont des artefacts hors pipeline TDD.
...
{
  "type": "analyse",
  "suivi": "docs/sprints/NN-<sujet>/00-suivi.md",
