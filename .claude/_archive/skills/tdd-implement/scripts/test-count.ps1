...
.DESCRIPTION
  Pensé pour les agents du pipeline (tdd-auto, ihm-builder, validation-visuelle) : le chemin
  vert ne renvoie qu'un objet `{ green, total, passed, failed, skipped, assemblies }`, pas
  des milliers de lignes. Sur rouge, ajoute `failures` (lignes d'échec, plafonnées) pour
  orienter sans tout déverser.
...
.EXAMPLE
  pwsh -NoProfile -File .claude/skills/tdd-implement/scripts/test-count.ps1
#>
[CmdletBinding()]
param(
    [string]$Filter,
